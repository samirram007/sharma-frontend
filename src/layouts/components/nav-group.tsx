      ...
      <SidebarMenuItem>
        <CollapsibleTrigger asChild>
          <SidebarMenuButton tooltip={item.title} isActive={isParentActive}>
            {item.icon && (
              <NavIcon icon={item.icon} color={item.color} isActive={isParentActive} />
            )}
            <span>{item.title}</span>
            {item.badge && <NavBadge>{item.badge}</NavBadge>}
                ...
                <SidebarMenuSubItem key={subItem.title}>
                  <SidebarMenuSubButton asChild isActive={isSubActive}>
                    <Link to={subItem.url} onClick={() => setOpenMobile(false)}>
                      {subItem.icon && (
                        <NavIcon icon={subItem.icon} color={subItem.color} isActive={isSubActive} />
                      )}
          ...
          {item.items.map((sub) => {
            ...
            return (
              <DropdownMenuItem key={`${sub.title}-${sub.url}`} asChild>
                ...
                >
                  {sub.icon && (
                    <NavIcon icon={sub.icon} color={sub.color} isActive={isSubActive} />
                  )}
