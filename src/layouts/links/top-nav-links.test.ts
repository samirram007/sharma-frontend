...
import { buildTopNavLinksFromTree, filterTopNavLinks, topNavLinks } from './top-nav-links'
...
describe('buildTopNavLinksFromTree', () => {
    it('keeps only root nodes flagged isTopMenu', () => {
        const links = buildTopNavLinksFromTree([
            node({ id: 1, menuName: 'Freight', route: '/transactions/freight', isTopMenu: true }),
        ...
        const links = buildTopNavLinksFromTree([
            node({
                ...
                children: [
                    ...
                    node({ id: 3, menuName: 'Receipt Book', route: '/reports/receipt_book' }),
                ],
            }),
        ])
    ...
    it('resolves DB icon names to icon components for links, submenu groups and items', () => {
        const links = buildTopNavLinksFromTree([
            node({
                ...
                isTopMenu: true,
                children: [
                    node({ id: 2, menuName: 'Day Book', route: '/reports/day_book', icon: 'Notebook' }),
                    node({ id: 3, menuName: 'Freight', route: '/reports/freight', icon: 'UnknownIcon' }),
                ],
            }),
            node({ id: 4, menuName: 'Freight', route: '/transactions/freight', icon: 'TruckDelivery', isTopMenu: true }),
        ])
