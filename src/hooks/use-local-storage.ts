 ...
 *
 * Pass `{ deserialize }` to transform the parsed stored value before use (e.g.
 * merge legacy data with the current defaults). The transformed value is also
 * what gets written back on the first change, normalizing older entries. If
 * the transform throws, the default is used.
 *
...
export function useLocalStorage<T>(
  ...
  options?: {
    raw?: boolean
    /** Transform the parsed stored value before use (e.g. merge with defaults). */
