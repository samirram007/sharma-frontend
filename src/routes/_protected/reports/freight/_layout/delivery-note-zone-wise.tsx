...
export const Route = createFileRoute(
    ...
    component: () => {
        const { data: zoneWiseData } = useSuspenseQuery(deliveryNoteZoneWiseQueryOptions())

        return <DeliveryNoteZoneWise data={zoneWiseData?.data} chartTitle='Delivery Notes per Zone' title='Delivery Note (Zone Wise)' />
    },
    errorComponent: (error: any) =>
        <div className='h-[50vh] no-scrollbar flex justify-center items-center flex-col gap-2'>
            <Card className='w-[400px] h-[200px] p-8 border-2 border-red-600 flex flex-col gap-4 justify-between items-between shadow-lg'>
                ...
                <pre className='text-sm text-red-600 overflow-auto max-h-20'>{error.error?.message ?? 'Unknown error'}</pre>
