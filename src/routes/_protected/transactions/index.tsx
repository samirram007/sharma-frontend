...
const quickActions = [
  {
    ...
    description: 'Track dispatch details, transporter info, and freight calculation.',
  ...
  {
    title: 'Delivery Note',
    description: 'Prepare outward movement records with party and item details.',
...
const relatedTopics = [
  {
    ...
    description: 'Open voucher categories and post transactional entries by type.',
    to: '/transactions/vouchers',
  },
  {
    title: 'Reports & Reconciliation',
    description: 'Review stock summary and day-book outcomes after posting vouchers.',
    to: '/reports/stock_summary',
  },
  {
    title: 'Day Book Drilldown',
    description: 'Analyze posting flow and verify transaction trails in one place.',
    to: '/reports/day_book',
  },
]
    ...
    <div className='space-y-6 p-4 sm:p-6'>
      <section className='overflow-hidden rounded-2xl border bg-card'>
        <div className='bg-[radial-gradient(circle_at_20%_20%,rgba(37,99,235,0.14),transparent_45%),radial-gradient(circle_at_80%_0%,rgba(16,185,129,0.14),transparent_35%)] p-6 sm:p-8'>
          ...
          <p className='mt-2 max-w-2xl text-sm text-muted-foreground sm:text-base'>
            Manage freight, voucher flow, and posting lifecycle from a single place.
            Use the quick actions below to jump into high-frequency transaction tasks.
          </p>
        </div>
      </section>
      ...
      <section>
        <div className='mb-3 flex items-center justify-between'>
          ...
          <span className='text-xs text-muted-foreground'>Most used modules</span>
        </div>
        <div className='grid grid-cols-1 gap-4 md:grid-cols-3'>
          {quickActions.map((action) => {
            const Icon = action.icon
            return (
              <Card key={action.title} className='border-border/70 transition hover:shadow-md'>
