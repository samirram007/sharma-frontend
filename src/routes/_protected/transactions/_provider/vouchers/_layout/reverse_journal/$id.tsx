...
export const Route = createFileRoute(
  '/_protected/transactions/_provider/vouchers/_layout/reverse_journal/$id',
)({
  params: {
  ...
  errorComponent: () => (
    <div>
      <span className="bg-red-400">By ID:</span> Error loading reverse journal
      voucher.
    </div>
  ),
...
function RouteComponent() {
  ...
  const { data: reverseJournal } = useSuspenseQuery(
    reverseJournalQueryOptions(id),
  )
  return (
