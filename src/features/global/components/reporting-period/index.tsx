...
import {
  formSchemaReportingPeriod,
  type ReportingPeriodForm,
} from '../../data/schema'
...
export const PeriodDetailsDialog = ({
  open,
  setopen,
  hideTrigger,
}: {
  ...
  const { mutate: saveReportingPeriod, isPending } =
    useReportingPeriodMutation()
  ...
  const form = useForm<ReportingPeriodForm>({
    resolver: zodResolver(
      formSchemaReportingPeriod,
    ) as Resolver<ReportingPeriodForm>,
    defaultValues: {
  ...
  const applyQuickRange = useCallback(
    (label: string, start: Date, end: Date) => {
      ...
      const fyStart = userFiscalYear?.fiscalYear?.startDate
        ? new Date(userFiscalYear.fiscalYear.startDate)
        : null
      const fyEnd = userFiscalYear?.fiscalYear?.endDate
        ? new Date(userFiscalYear.fiscalYear.endDate)
        : null
      ...
      form.setValue('startDate', fmtStart as unknown as Date, {
        shouldValidate: true,
        shouldDirty: true,
      })
      form.setValue('endDate', fmtEnd as unknown as Date, {
        shouldValidate: true,
        shouldDirty: true,
      })
  ...
  const moreRanges: QuickRange[] = [
    ...
    },
    {
      ...
      getRange: () => {
        const fyStart = userFiscalYear?.fiscalYear?.startDate
          ? startOfDay(new Date(userFiscalYear.fiscalYear.startDate))
          : startOfMonth(new Date())
        const fyEnd = userFiscalYear?.fiscalYear?.endDate
          ? startOfDay(new Date(userFiscalYear.fiscalYear.endDate))
          : startOfDay(new Date())
...
const DateBox = (props: DateBoxProps) => {
  ...
  const validateFiscalYear = (date: Date): boolean => {
    ...
    if (
      ...
      toast.error(
        `Date must be within fiscal year (${date_format(fyStart)} to ${date_format(fyEnd)})`,
      )
      return false
    }
    return true
  ...
  const applyDate = (date: Date) => {
    ...
    form.setValue(name, DBFormat as unknown as Date, {
      shouldValidate: true,
      shouldDirty: true,
    })
  }
