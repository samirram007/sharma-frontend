      ...
      <Main>
        <div className="flex flex-col items-center justify-center gap-4 py-24">
          <p className="text-muted-foreground">
            Error loading user-wise dashboard data.
          </p>
        ...
        <Card>
          ...
          <CardContent>
            {isLoading && !data ? (
              ...
              <div className="text-2xl font-bold">
                {formatNumber(totals.vouchers)}
              </div>
            )}
            <p className="text-xs text-muted-foreground">
              {totals.deliveries} delivery · {totals.receipts} receipt ·{' '}
              {totals.freights} freight
            </p>
          </CardContent>
        </Card>
        <Card>
          ...
          <CardContent>
            {isLoading && !data ? (
              ...
              <div className="text-2xl font-bold">
                {formatNumber(
                  totals.users > 0
                    ? Math.round(totals.vouchers / totals.users)
                    : 0,
                )}
              </div>
            )}
      ...
      <div className="mt-4 grid grid-cols-1 gap-4 lg:grid-cols-7">
        <Card className="col-span-1 lg:col-span-4">
          <CardHeader>
            ...
            <CardDescription>
              Number of entries created by each user
            </CardDescription>
          </CardHeader>
          ...
          {rows.map((row) => (
            ...
            >
              ...
              <td className="p-3 text-right tabular-nums">
                {formatNumber(row.totalVouchers ?? 0)}
              </td>
              <td className="p-3 text-right tabular-nums">
                {formatNumber(row.delivery_notes ?? 0)}
              </td>
              <td className="p-3 text-right tabular-nums">
                {formatNumber(row.receipt_notes ?? 0)}
              </td>
              <td className="p-3 text-right tabular-nums">
                {formatNumber(row.freights ?? 0)}
              </td>
