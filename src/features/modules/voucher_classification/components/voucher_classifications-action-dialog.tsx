...
} from '@/features/modules/voucher_classification/data/schema'
...
import {
  storeVoucherClassificationService,
  updateVoucherClassificationService,
} from '../data/api'
...
export function VoucherClassificationsActionDialog({
  currentRow,
  open,
  onOpenChange,
}: Props) {
  ...
  const mutateVoucherClassification = useMutation({
    mutationFn: async (data: VoucherClassificationForm) => {
      if (isEdit && currentRow) {
        return await updateVoucherClassificationService({
          ...data,
          id: currentRow.id,
        })
      } else if (!isEdit) {
    ...
    >
      <DialogContent className="sm:max-w-xl">
        <DialogHeader className="text-left border-b pb-2">
          <DialogTitle>
            {isEdit
              ? 'Edit Voucher Classification'
              : 'Add New Voucher Classification'}
          </DialogTitle>
          <DialogDescription>
            {isEdit
              ? 'Update the voucher classification here. '
              : 'Create new voucher classification here. '}
            ...
            >
              ...
              <div className="space-y-2">
                ...
                <FormInputField
                  ...
                  name="inclusionRules"
                  label=""
                  placeholder='e.g. {"groups": [1, 2], "ledgers": [5]}'
                />
              </div>
              <div className="space-y-2">
                <label className="text-sm font-medium">
