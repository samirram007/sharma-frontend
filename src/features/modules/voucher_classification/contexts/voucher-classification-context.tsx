...
interface VoucherClassificationContextType {
  ...
  setCurrentRow: React.Dispatch<
    React.SetStateAction<VoucherClassification | null>
  >
}

const VoucherClassificationContext =
  React.createContext<VoucherClassificationContextType | null>(null)
...
export default function VoucherClassificationProvider({ children }: Props) {
  ...
  const [currentRow, setCurrentRow] = useState<VoucherClassification | null>(
    null,
  )
...
export const useVoucherClassification = () => {
  const voucherClassificationContext = React.useContext(
    VoucherClassificationContext,
  )

  if (!voucherClassificationContext) {
    throw new Error(
      'useVoucherClassification has to be used within <VoucherClassificationContext>',
    )
  }
