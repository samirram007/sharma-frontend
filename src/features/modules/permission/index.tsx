...
function PermissionPanel({ roleId }: { roleId: number }) {
  const {
    ...
    refetch,
    isFetching,
  } = useSuspenseQuery(featuresWithRolePermissionsQueryOptions(roleId))
      ...
      {stats.notSet === stats.totalFeatures && stats.totalFeatures > 0 && (
        <Alert className="border-amber-200 bg-amber-50 dark:border-amber-800 dark:bg-amber-950/30">
          <AlertDescription className="text-sm text-amber-700 dark:text-amber-400">
            <strong>No permissions configured yet.</strong> Use the toggle
            switches below to grant or deny access to each feature for this
            role.
          </AlertDescription>
        </Alert>
      )}
  ...
  return (
    <div className="flex flex-col items-center justify-center rounded-xl border bg-card shadow-sm py-16 text-center">
      ...
      <p className="mt-1 max-w-sm text-sm text-muted-foreground">
        Choose a role from the sidebar to view and configure its feature
        permissions.
      </p>
    </div>
  )
