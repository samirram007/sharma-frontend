...
import { ChevronDown, ChevronRight, Search, Check, X, CheckCheck, Ban } from 'lucide-react'
...
export function PermissionMatrix({ features, roleId }: PermissionMatrixProps) {
  ...
  const moduleGroups = useMemo(() => {
    const groups = new Map<number, { module: any; features: FeatureWithPermission[] }>()
  ...
  const filteredGroups = useMemo(() => {
    ...
    const result: { moduleId: number; module: any; features: FeatureWithPermission[] }[] = []
    for (const [moduleId, group] of moduleGroups) {
      if (!q) {
        result.push({ moduleId, module: group.module, features: group.features })
      } else {
  ...
  const handleToggle = useCallback(
    (feature: FeatureWithPermission) => {
      ...
      if (rp?.id) {
        ...
        savePermission({ ...rp, isAllowed: !rp.isAllowed, isEdit: true, roleId, appModuleFeatureId: feature.id })
      } else {
        // No permission yet — create one as granted
        savePermission({ roleId, appModuleFeatureId: feature.id, isAllowed: true, isEdit: false } as any)
      }
    },
      ...
      for (const feature of moduleFeatures) {
        ...
        if (rp?.id) {
          if (rp.isAllowed !== allow) {
            savePermission({ ...rp, isAllowed: allow, isEdit: true, roleId, appModuleFeatureId: feature.id })
          }
        } else if (allow) {
          savePermission({ roleId, appModuleFeatureId: feature.id, isAllowed: true, isEdit: false } as any)
        }
  ...
  return (
    <div className='flex flex-col rounded-xl border bg-card shadow-sm overflow-hidden'>
      ...
      <div className=' flex flex-row flex-nowrap items-center gap-2 border-b bg-muted/30 px-4 py-1'>
        ...
        />
        {search && (
          <Button variant='ghost' size='sm' onClick={() => setSearch('')} className='h-7 px-2 text-xs'>
            Clear
          </Button>
        )}
      </div>
      ...
      <ScrollArea className='h-[calc(100vh-520px)] min-h-[300px]'>
        <div className='p-3 space-y-2'>
          ...
          {filteredGroups.map(({ moduleId, module: mod, features: groupFeatures }) => {
            const isExpanded = expandedModules.has(moduleId)
            const granted = groupFeatures.filter((f) => f.rolePermission?.isAllowed).length
            const denied = groupFeatures.filter((f) => f.rolePermission && !f.rolePermission.isAllowed).length
            const total = groupFeatures.length

            return (
              <Collapsible key={moduleId} open={isExpanded} onOpenChange={() => toggleModule(moduleId)}>
                <div className='rounded-lg border bg-card overflow-hidden transition-all hover:shadow-sm'>
                      ...
                      <div className='hidden sm:flex items-center gap-2'>
                        <div className='flex h-1.5 w-24 overflow-hidden rounded-full bg-muted'>
                          ...
                          <div
                            ...
                            style={{ width: total > 0 ? `${(denied / total) * 100}%` : '0%' }}
                          />
                        </div>
                          ...
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <Button
                                ...
                                onClick={() => handleModuleBulk(groupFeatures, true)}
                              >
                                <CheckCheck className='h-3.5 w-3.5' />
                              </Button>
                            </TooltipTrigger>
                            <TooltipContent>Grant all in {mod?.name ?? 'module'}</TooltipContent>
                          </Tooltip>
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <Button
                                ...
                                onClick={() => handleModuleBulk(groupFeatures, false)}
                              >
                                <Ban className='h-3.5 w-3.5' />
                              </Button>
                            </TooltipTrigger>
                            <TooltipContent>Deny all in {mod?.name ?? 'module'}</TooltipContent>
                          </Tooltip>
                        </TooltipProvider>
                      </div>
                    </div>
                  ...
                  {/* Feature rows */}
                  <CollapsibleContent>
                    <div className='border-t divide-y'>                        {groupFeatures.map((feature: FeatureWithPermission) => {
                        const rp = feature.rolePermission
                        ...
                        return (
                          <div
                            ...
                            className={cn(
                              ...
                              'hover:bg-accent/20',
                              isAllowed === true && 'bg-emerald-50/40 dark:bg-emerald-950/10',
                              isAllowed === false && 'bg-red-50/30 dark:bg-red-950/10'
                            )}
                          >
                            {/* Status icon */}
                            ...
                            <div className='flex-1 min-w-0'>
                              ...
                              <p className='text-xs text-muted-foreground font-mono truncate'>{feature.code}</p>
                            </div>
                            ...
                            <Badge
                              ...
                              className={cn(
                                ...
                                isAllowed === true && 'border-emerald-300 text-emerald-700 bg-emerald-50 dark:border-emerald-700 dark:text-emerald-400 dark:bg-emerald-950/30',
                                isAllowed === false && 'border-red-300 text-red-600 bg-red-50 dark:border-red-700 dark:text-red-400 dark:bg-red-950/30',
                                isAllowed === null && 'border-muted-foreground/20 text-muted-foreground bg-muted/50'
                              )}
                            >
                              {isAllowed === true ? 'Granted' : isAllowed === false ? 'Denied' : 'Not Set'}
                            </Badge>
