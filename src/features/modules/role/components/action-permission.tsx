...
const ModalAction = ({
  ...
  const modules = useMemo(() => {
    return modulesData.data?.filter(
      (module: AppModule) => module.status === 'active',
    )
  }, [modulesData])
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="  sm:max-w-lg md:max-w-2xl lg:max-w-5xl xl:max-w-7xl">
        <DialogHeader className="text-left">
          ...
          <DialogDescription>
            Update the role permission here. Click save when you&apos;re done.
          </DialogDescription>
        </DialogHeader>
        ...
        <DialogFooter className="flex justify-between items-center">
          {/* //instruction */}
          <div className="w-full text-muted-foreground text-left">
            Select Module here..
          </div>
        </DialogFooter>
      </DialogContent>
