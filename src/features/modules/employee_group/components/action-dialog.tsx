...
export function ActionDialog({ currentRow, open, onOpenChange }: Props) {
  ...
  const onSubmit = (values: EmployeeGroupForm) => {
    ...
    saveEmployeeGroup(currentRow ? { ...values, id: currentRow.id } : values)
    onOpenChange(false)
  }
    ...
    >
      <DialogContent className="sm:max-w-lg">
        <DialogHeader className="text-left border-b-2 pb-2">
          <DialogTitle>
            {isEdit ? 'Edit Employee Group' : 'Add New Employee Group'}
          </DialogTitle>
          <DialogDescription>
            {isEdit
              ? 'Update the Employee Group here. '
              : 'Create new Employee Group here. '}
