...
const AccountGroupDropdown = (props: Props) => {
  ...
  const accountNature: AccountNature | null = useMemo(() => {
    ...
    return (
      accountGroupList.data.find(
        (group: AccountGroup) => group.id === Number(accountGroupId),
      )?.accountNature || null
    )
  ...
  const handleValueChange = (value: string) => {
    ...
    form.setValue(
      ...
      Number(
        accountGroupList.data.find(
          (group: AccountGroup) => group.id === Number(value),
        )?.accountNatureId,
      ),
    )
  }
          ...
          <FormItem className="grid grid-cols-6 items-start space-y-0 gap-x-4 gap-y-1 ">
            ...
            <div className="w-full col-span-4 space-y-1">
              <SelectDropdown
                ...
                items={[
                  nullItem,
                  ...(accountGroupList?.data.map(
                    (accountGroup: AccountGroup) => ({
                      label:
                        capitalizeAllWords(accountGroup.name) +
                        String(accountGroup.accountNatureId),
