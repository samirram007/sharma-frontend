...
import { useMarkAllAsRead, useMarkAsRead, useMyNotifications, useUnreadCount } from '../data/queryOptions'
...
function NotificationItem({
  ...
  const typeStyles: Record<string, string> = {
    ...
    error: 'border-l-4 border-l-red-400 dark:border-l-red-500 bg-red-50/50 dark:bg-red-950/20',
    ...
    >
      ...
      {!notification.isRead && (
        <Button
          ...
          title='Mark as read'
        >
          <IconCheck size={14} className='text-slate-400 hover:text-emerald-500' />
        </Button>
      )}
    </div>
  )
...
export function NotificationBell() {
  ...
  const { data: notifications = [], isLoading: isNotificationsLoading } = useMyNotifications(1, 10)
  ...
  return (
    <DropdownMenu modal={false}>
      <DropdownMenuTrigger asChild>
        ...
        >
          {unread > 0 ? (
            <IconBellFilled size={18} className='text-amber-500 dark:text-amber-400' />
          ) : (
            <IconBell size={18} />
          )}
          ...
          <DropdownMenuGroup>
            ...
            {!isLoading && notifications.length === 0 && (
              <div className='flex flex-col items-center justify-center py-8 text-center'>
                <IconBell size={28} className='text-slate-300 dark:text-slate-600' />
