...
export async function fetchOpenPreviewService(
  newFiscalYearId: number,
  previousFiscalYearId: number,
) {
  return await getData(
    `fiscal-years/${newFiscalYearId}/open-preview/${previousFiscalYearId}`,
  )
}

export async function openFiscalYearService(
  newFiscalYearId: number,
  previousFiscalYearId: number,
) {
