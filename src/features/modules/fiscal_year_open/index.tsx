...
import {
  openPreviewQueryOptions,
  useOpenFiscalYearMutation,
...
export default function FiscalYearOpen() {
  ...
  const { id } = useParams({
    from: '/_protected/masters/organization/_layout/fiscal_year/_layout/$id/open',
  })
  ...
  useEffect(() => {
    if (
      isNewMode &&
      userFiscalYear?.fiscalYearId &&
      userFiscalYear?.fiscalYear?.status === 'active'
    ) {
      navigate({
    ...
    isNewMode,
    userFiscalYear?.fiscalYearId,
    userFiscalYear?.fiscalYear?.status,
    navigate,
  ])
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="space-y-1">
            ...
            <p className="text-muted-foreground">
              Choose the fiscal year to open. Balances from its previous closed
              ...
              <Card key={fy.id} className="transition-shadow hover:shadow-md">
                <CardHeader className="pb-2">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <Badge
                      variant={
                        fy.status === 'active' ? 'default' : 'destructive'
                      }
                    >
    ...
    <Main className="max-w-6xl mx-auto space-y-6 py-6">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="space-y-1">
          ...
          <p className="text-muted-foreground">
            Carry forward balances from the previous fiscal year to{' '}
            {newFy?.name ?? `FY #${newFiscalYearId}`}
          </p>
        </div>
        <Button
          variant="outline"
          onClick={() => navigate({ to: '/masters/organization/fiscal_year' })}
        >
          <IconEye className="mr-2 h-4 w-4" />
          Back to Fiscal Years
        </Button>
      ...
      {!prevFyId && step === 'preview' && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Choose the closed fiscal year from which to carry forward balances
              and stock.
            </CardDescription>
          </CardHeader>
          <CardContent>
                ...
                <SelectContent>
                  {fiscalYears.map((fy) => (
                    <SelectItem key={fy.id} value={String(fy.id)}>
                      {fy.name} ({formatFyDate(fy.startDate)} —{' '}
                      {formatFyDate(fy.endDate)})
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
      ...
      {prevFyId && isError && !preview && (
        <div className="text-center space-y-4 py-12">
          ...
          <p className="text-lg font-medium text-destructive">
            Failed to load opening preview
          </p>
        ...
        <>
              ...
              <CardHeader className="pb-2">
                <div className="flex items-center justify-between">
                  <CardTitle className="text-2xl font-bold">
                    {preview.totalLedgers}
                  </CardTitle>
                  <IconDatabase className="h-5 w-5 text-muted-foreground" />
                </div>
                <CardDescription>
                  Balance Sheet Ledgers from{' '}
                  <strong>{preview.previousFiscalYear.name}</strong>
                </CardDescription>
              </CardHeader>
            </Card>
            <Card className="transition-shadow hover:shadow-md">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                These ledger balances will be carried forward to{' '}
                {preview.newFiscalYear.name}
              </CardDescription>
            </CardHeader>
            <CardContent>
                        ...
                        >
                          ...
                          <td className="py-2">
                            <Badge
                              variant="outline"
                              className={
                                getNatureBadge(ledger.nature).className
                              }
                            >
                    ...
                    <tfoot>
                      <tr className="border-t-2 border-primary/30">
                        ...
                        <td className="py-2 font-semibold">
                          {preview.totalLedgers} ledgers
                        </td>
                        <td className="py-2 text-right font-mono font-semibold">
                          {preview.balanceSheetLedgers
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                These stock quantities will be carried forward to{' '}
                {preview.newFiscalYear.name}
              </CardDescription>
            </CardHeader>
            <CardContent>
                    ...
                    >
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <Badge variant="secondary">
                          {item.totalQuantity.toFixed(2)} total qty
                        </Badge>
                      </div>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="space-y-2">
              <p>
                This will create a single <strong>OpeningJournal</strong>{' '}
                voucher that carries forward:
              </p>
              <ul className="list-disc pl-5 text-sm space-y-1">
                <li>
                  <strong>
                    {preview?.totalLedgers ?? 0} balance sheet ledgers
                  </strong>{' '}
                  — Assets, Liabilities, and Equity
                </li>
                <li>
                  <strong>{preview?.totalStockItems ?? 0} stock items</strong> —
                  with godown-wise quantities
                </li>
              </ul>
