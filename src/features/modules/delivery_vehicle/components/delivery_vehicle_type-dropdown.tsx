...
const DeliveryVehicleTypeDropdown = (props: Props) => {
  ...
  const deliveryVehicleTypeOptions: readonly DeliveryVehicleType[] =
    DeliveryVehicleTypes
  return (
    <FormField
      ...
      render={({ field }) => (
        <FormItem
          className={cn(
            gapClass
              ? gapClass
              : 'grid grid-cols-2 items-center space-y-0 gap-x-4 gap-y-1',
          )}
        >
          <FormLabel className="text-right">Delivery Vehicle Type</FormLabel>
          <SelectDropdown
