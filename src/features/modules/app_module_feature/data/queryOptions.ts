...
import {
  deleteAppModuleFeatureService,
  fetchAppModuleFeatureByIdService,
  fetchAppModuleFeatureService,
  storeAppModuleFeatureService,
  updateAppModuleFeatureService,
} from './api'
import { getData } from '@/utils/dataClient'
const Key = 'AppModuleFeatures'
const BASE_KEY = 'AppModuleFeatures'

export const appModuleFeatureRoleQueryOptions = (
  id?: number,
  moduleid?: number,
) => {
  return queryOptions({
...
export const appModuleFeatureQueryOptions = (id?: number) => {
  return queryOptions({
    ...
    queryFn: () => {
      return id
        ? fetchAppModuleFeatureByIdService(id)
        : fetchAppModuleFeatureService()
    },
    staleTime: 1000 * 60 * 5, // 5 minutes
    retry: 1,
