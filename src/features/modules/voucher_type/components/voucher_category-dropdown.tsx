...
const VoucherCategoryDropdown = (props: Props) => {
  ...
  const voucherCategoryId = form.watch('voucherCategoryId') as
    string | number | undefined // Watch form value for reactivity
  const voucherCategory: VoucherCategory | null = useMemo(() => {
    if (!voucherCategoryList?.data) return null
    return (
      voucherCategoryList.data.find(
        (group: VoucherCategory) => group.id === Number(voucherCategoryId),
      ) || null
    )
          ...
          <FormItem className="grid grid-cols-6 items-start space-y-0 gap-x-4 gap-y-1 ">
            ...
            <div className="w-full flex gap-2 flex-row items-center justify-start col-span-4 space-y-1">
              <SelectDropdown
                ...
                items={voucherCategoryList?.data.map(
                ...
                <HoverCard>
                  <HoverCardTrigger>
                    <div className="text-muted-foreground  ">
                      <InfoIcon className="cursor-pointer" size={24} />{' '}
                    </div>
                  </HoverCardTrigger>
                  <HoverCardContent>
