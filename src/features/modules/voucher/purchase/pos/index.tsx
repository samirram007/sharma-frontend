...
const buildEditValues = (row: PurchaseVoucher): PurchaseVoucherForm => ({
...
const Pos = ({ currentRow, hidePartyLedger = false }: PurchaseProps) => {
  ...
  const { mutate: savePurchase, isPending: isSaving } =
  ...
  const [partyLedgerStr, setPartyLedgerStr] = useState<string>(
    hidePartyLedger
      ? '0'
      : currentRow?.partyLedger?.id
        ? String(currentRow.partyLedger.id)
        : '',
  )
  ...
  useEffect(() => {
    if (
      (hidePartyLedger
        ? Boolean(txLedgerStr)
        : Boolean(partyLedgerStr && txLedgerStr)) &&
      itemFields.length === 0
    ) {
      addItemRow()
    }
  ...
  const canShowItems = hidePartyLedger
    ? Boolean(txLedgerStr)
    : Boolean(partyLedgerStr && txLedgerStr)
      ...
      <Form {...mainForm}>
        ...
        >
          <div className="border-b border-slate-300 px-3 py-2 space-y-2">
            ...
            <div
              className={`grid gap-2 ${hidePartyLedger ? 'lg:grid-cols-1' : 'lg:grid-cols-2'}`}
            >
              {!hidePartyLedger ? (
                <div>
                  <div className="mb-1 text-[11px] font-semibold uppercase text-slate-500">
            ...
            {canShowItems ? (
              ...
              <div className="flex flex-col items-center justify-center py-8 text-center">
                ...
                <p className="text-slate-500 mb-4">
                  {hidePartyLedger
                    ? 'Please select <span className="font-medium">Purchase Account</span> to add item entries.'
                    : 'Please select both <span className="font-medium">Supplier</span> and <span className="font-medium">Purchase Account</span> to add item entries.'}
                </p>
