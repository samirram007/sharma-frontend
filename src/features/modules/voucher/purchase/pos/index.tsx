...
import { usePurchaseVoucherMutation } from '../data/queryOptions'
import {
...
import type { PurchaseProps } from './contracts'
...
const buildEditValues = (
    ...
    voucherEntries:
        row.voucherEntries?.length > 0
            ...
            : PurchaseDefaultValues.voucherEntries,
...
const Pos = ({ currentRow, hidePartyLedger = false }: PurchaseProps) => {
    const isEdit = Boolean(currentRow?.id)
    const { setMovementType } = usePos()
    const { mutate: savePurchase, isPending: isSaving } = usePurchaseVoucherMutation()

    const { data: supplierLedgersResponse, isPending: isSuppliersLoading } = useQuery({
        queryKey: ['accountLedgers', 'supplier_ledgers'],
        queryFn: () => fetchPartyLedgerService('supplier_ledgers'),
        enabled: !hidePartyLedger,
    })
    const { data: purchaseLedgersResponse, isPending: isPurchaseLedgersLoading } = useQuery({
    ...
    const supplierItems = useMemo(
        () => supplierLedgers.map((l) => ({ label: l.name ?? '', value: String(l.id ?? '') })),
        [supplierLedgers]
    )
    const purchaseLedgerItems = useMemo(
        () => purchaseLedgers.map((l) => ({ label: l.name ?? '', value: String(l.id ?? '') })),
        [purchaseLedgers]
    )

    const [partyLedgerStr, setPartyLedgerStr] = useState<string>(
        hidePartyLedger ? '0' : currentRow?.partyLedger?.id ? String(currentRow.partyLedger.id) : ''
    )
    ...
    const mainForm = useForm<PurchaseVoucherForm>({
        resolver: zodResolver(PurchaseFormSchema) as Resolver<PurchaseVoucherForm>,
        defaultValues:
            isEdit && currentRow
                ? buildEditValues(currentRow)
                : { ...PurchaseDefaultValues, isEdit: false },
    })
    ...
    useEffect(() => {
        if (hidePartyLedger) {
            mainForm.setValue('partyLedger', null)
            mainForm.setValue('party', null)
            return
        }
    ...
    useEffect(() => { setMovementType?.('in') }, [setMovementType])

    // initialise with one empty row when accounts are selected
    useEffect(() => {
        if ((hidePartyLedger ? Boolean(txLedgerStr) : Boolean(partyLedgerStr && txLedgerStr)) && itemFields.length === 0) {
            addItemRow()
        }
    }, [hidePartyLedger, partyLedgerStr, txLedgerStr, itemFields.length])

    const canShowItems = hidePartyLedger ? Boolean(txLedgerStr) : Boolean(partyLedgerStr && txLedgerStr)

    const onSubmit = (values: PurchaseVoucherForm) => {
        ...
        savePurchase(
            ...
            },
            {
                onSuccess: () => {
                    if (!isEdit) {
                        mainForm.reset({ ...PurchaseDefaultValues, isEdit: false })
                ...
                >
                    <div className='border-b border-slate-300 px-3 py-2 space-y-2'>
                        <div className='flex items-center justify-between'>
                            <h2 className='text-base font-semibold text-slate-800'>Purchase</h2>
                            <p className='text-xs text-slate-500'>Voucher Type ID: 1005</p>
                        </div>
                        ...
                        <div className={`grid gap-2 ${hidePartyLedger ? 'lg:grid-cols-1' : 'lg:grid-cols-2'}`}>
                            {!hidePartyLedger ? (
                                <div>
                                    <div className='mb-1 text-[11px] font-semibold uppercase text-slate-500'>Party A/c (Supplier)</div>
                                    <SelectDropdown
                                        defaultValue={partyLedgerStr}
                                        onValueChange={(val) => {
                                            setPartyLedgerStr(val)
                                            const ledger = supplierLedgers.find((l) => String(l.id) === val)
                                            if (ledger) {
                            ...
                            <div>
                                <div className='mb-1 text-[11px] font-semibold uppercase text-slate-500'>Purchase A/c</div>
                                <SelectDropdown
                                    defaultValue={txLedgerStr}
                                    onValueChange={(val) => {
                                        setTxLedgerStr(val)
                                        const ledger = purchaseLedgers.find((l) => String(l.id) === val)
                                        if (ledger) {
                    ...
                    <div className='min-h-0 overflow-auto'>
                        {canShowItems ? (
                                    ...
                                    {additionalEntries.length === 0 ? (
                                        ...
                                        <div className='space-y-2'>
                                            {additionalEntries.map((row) => (
                                                <div key={row.id} className='grid gap-2 rounded border border-slate-200 p-2 lg:grid-cols-[minmax(220px,1fr)_110px_120px_minmax(180px,1fr)_80px]'>
                                                    <SelectDropdown
                            ...
                            <div className='flex h-full items-center justify-center text-slate-400 text-sm'>
                                {hidePartyLedger
                                    ? 'Please select Purchase Account to add item entries.'
                                    : 'Please select both Supplier and Purchase Account to add item entries.'}
                            </div>
                        )}
                    </div>
                    ...
                    <div className='flex items-center justify-between border-t border-slate-300 px-3 py-2'>
                        <div className='space-y-1 text-sm text-slate-600'>
                            <div>
                                Item Total: <span className='font-semibold text-slate-900'>{total.toFixed(2)}</span>
                            </div>
                            <div>
                                Dr {totalDebit.toFixed(2)} | Cr {totalCredit.toFixed(2)}
                                <span className={`ml-2 rounded px-2 py-0.5 text-xs font-semibold ${isBalanced ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}>
...
type DateBoxProps = {
    form: UseFormReturn<PurchaseVoucherForm>
    name: 'voucherDate'
}
