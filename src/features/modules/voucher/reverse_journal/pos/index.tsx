...
const buildEditValues = (
  row: ReverseJournalVoucher,
): ReverseJournalVoucherForm => ({
...
const getEntryType = (
  entry: ReverseJournalVoucherForm['voucherEntries'][number],
...
const Pos = ({ currentRow }: ReverseJournalProps) => {
  ...
  const { mutate: saveReverseJournal, isPending: isSaving } =
  ...
  const mainForm = useForm<ReverseJournalVoucherForm>({
    resolver: zodResolver(
      reverseJournalFormSchema,
    ) as Resolver<ReverseJournalVoucherForm>,
    defaultValues:
      isEdit && currentRow
