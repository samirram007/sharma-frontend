...
export function usePurchaseOrderVoucherMutation() {
    ...
    return useMutation({
        mutationFn: async (data: PurchaseOrderVoucherMutationPayload) => {
            if (data.id) {
                return await updatePurchaseOrderService(data as unknown as Record<string, unknown>)
            }
            return await storePurchaseOrderService(data as unknown as Record<string, unknown>)
        },
        onSuccess: () => {
