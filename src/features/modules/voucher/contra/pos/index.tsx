...
import { buildContraVoucherEntries, calculateTotalCredit } from '../test/contra-payload'
import {
...
const getLedgerCurrentBalance = (ledger: AccountLedger | undefined) => {
    if (!ledger) return 0
    const maybeBalance = (ledger as unknown as { currentBalance?: number | null }).currentBalance
...
const Pos = ({ currentRow }: ContraProps) => {
    ...
    const { data: accountLedgerResponse, isPending: isAccountLedgersLoading } = useQuery(accountLedgerQueryOptions())
    ...
    const initialDebitLedgerId = useMemo(() => {
        ...
        const debitEntry = currentRow?.voucherEntries?.find((entry) => Number(entry?.debit || 0) > 0)
        return debitEntry?.accountLedgerId ? String(debitEntry.accountLedgerId) : ''
    }, [currentRow])

    const [debitLedgerId, setDebitLedgerId] = useState<string>(initialDebitLedgerId)
    ...
    const mainForm = useForm<ContraVoucherForm>({
        ...
        defaultValues: isEdit && currentRow ? buildEditValues(currentRow) : { ...contraDefaultValues, isEdit: false },
    })
    ...
    const totalCredit = useMemo(() => calculateTotalCredit(watchedEntries), [watchedEntries])

    const hasCreditEntries = useMemo(
        () => watchedEntries.some((entry) => Number(entry?.accountLedgerId || 0) > 0 && Number(entry?.credit || 0) > 0),
        [watchedEntries]
    )

    const isBalanced = Boolean(debitLedgerId) && totalCredit > 0 && hasCreditEntries
    ...
    const onSubmit = (values: ContraVoucherForm) => {
        const selectedDebitLedger = cashBankLedgers.find((ledger) => String(ledger.id) === debitLedgerId)
    ...
    return (
        <div className='voucher-entry h-[calc(100dvh-170px)] w-full overflow-hidden border border-slate-300 bg-white text-[13px] text-slate-900'>
            <Form {...mainForm}>
                <form onSubmit={mainForm.handleSubmit(onSubmit)} className='grid h-full grid-rows-[auto_1fr_auto]'>
                    <div className='border-b border-slate-300 px-3 py-2'>
                        <div className='mb-2 flex items-center justify-between'>
                            <div>
                                <h2 className='text-base font-semibold text-slate-800'>Contra Voucher</h2>
                                <p className='text-xs text-slate-500'>Header debit account with credit line entries</p>
                            </div>
                        ...
                        <div className='space-y-2'>
                            <div className='grid gap-2 lg:grid-cols-[180px_180px_minmax(280px,1fr)_auto_auto]'>
                                <div>
                                    <div className='mb-1 text-[11px] font-semibold uppercase text-slate-500'>Contra No</div>
                                    <input
                                ...
                                <Button type='submit' className='h-9 self-end px-4' disabled={!isBalanced || isSaving}>
                            ...
                            <div className='grid gap-2 lg:grid-cols-[180px_180px_minmax(280px,1fr)_auto_auto]'>
                                <div className='max-w-md lg:col-start-1 lg:col-span-2'>
                                    <div className='mb-1 text-[11px] font-semibold uppercase text-slate-500'>Header Debit Account</div>
                                    <SelectDropdown
                                    ...
                                    <div className='mt-1 text-xs text-slate-500'>
                                        {formatCurrentBalance(getLedgerCurrentBalance(selectedHeaderLedger))}
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <table className='w-full table-fixed border-collapse'>
                            <thead>
                                <tr className='border-b border-slate-300 bg-slate-50 text-xs uppercase tracking-wide text-slate-600'>
                                    <th className='w-24 px-2 py-2 text-center font-semibold'>Type</th>
                            ...
                            <tbody>
                                {fields.map((field, index) => {
                                    ...
                                    const selectedLineLedger = cashBankLedgers.find(
                                        (ledger) => String(ledger.id) === String(entry?.accountLedgerId || '')
                                    )

                                    return (
                                        <tr key={field.id} className='border-b border-slate-200'>
                                            <td className='px-2 py-2 text-center'>
                                                <span className='inline-flex rounded bg-slate-100 px-2 py-1 text-xs font-semibold text-slate-600'>Credit</span>
                                            </td>
                                            <td className='px-3 py-2'>
                                                <div>
                                                    <SelectDropdown
                                                        ...
                                                        onValueChange={(value) =>
                                                            mainForm.setValue(`voucherEntries.${index}.accountLedgerId`, Number(value), { shouldDirty: true })
                                                        }
                                                        items={accountLedgerItems}
                                                        isPending={isAccountLedgersLoading}
                                                    ...
                                                    />
                                                    <div className='mt-1 text-xs text-slate-500'>
                                                        {formatCurrentBalance(getLedgerCurrentBalance(selectedLineLedger))}
                                                    </div>
                                                </div>
                                            </td>
                    ...
                    <div className='grid grid-cols-1 gap-2 border-t border-slate-300 px-3 py-2 sm:grid-cols-[1fr_auto]'>
                        <div>
                            ...
                            <Textarea
                                ...
                                onChange={(event) => mainForm.setValue('remarks', event.target.value, { shouldDirty: true })}
                        ...
                        <div className='flex min-w-48 flex-col justify-end text-right'>
                            ...
                            <div className='text-lg font-semibold text-slate-900'>{totalCredit.toFixed(2)}</div>
                        </div>
                    </div>
                </form>
            </Form>
