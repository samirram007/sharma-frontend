...
export const StockItemCombobox = ({
  ...
  const handleSelect = (value: string) => {
    ...
    form.setValue(
      `discount`,
      ((form.getValues(`rate`)! * form.getValues(`discountPercentage`)!) /
        100) *
        selected?.standardCost! *
        quantity,
    )
    form.setValue(
      `amount`,
      Number(
        (
          selected?.standardCost! * quantity -
    ...
    form.setValue(
      `stockItem`,
      selected ?? null,
      { shouldValidate: true, shouldDirty: true }, // optional but recommended
    )

    // setValue(value);
    setOpen(false)
    requestAnimationFrame(() => focusNextFocusable())
  }
              ...
              {frameworks.map((framework) => (
                ...
                >
                  ...
                  <div className="flex flex-row justify-between w-full">
                    ...
                    <div>
                      {Number(framework.stockInHand).toFixed(
                        framework.noOfDecimalPlaces,
                      )}{' '}
