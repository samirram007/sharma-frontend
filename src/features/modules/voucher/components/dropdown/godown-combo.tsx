...
export const GodownCombobox = ({ godowns, handleRemove }: GodownComboboxProps) => {
    ...
    const noOfDecimalPlaces = (stockItem as any)?.stockUnit?.noOfDecimalPlaces ?? 2

    const stockMap = React.useMemo(() => {
        const map: Record<string, string> = {};
        godownItemStocks?.forEach((row: any) => {
            map[row.godownId] = Number(row.stockInHand || 0).toFixed(noOfDecimalPlaces);
        });
        return map;
    ...
    const frameworks = [
        ...
        ...(godowns
            ...
            ?.map((godown: Godown) => {
            const stock = stockMap[godown.id] ?? Number(0).toFixed(noOfDecimalPlaces);
