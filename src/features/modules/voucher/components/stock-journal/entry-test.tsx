...
export const StockJournalEntry = (props: StockJournalEntryProps) => {
  ...
  const stockJournalEntryForm = useForm<StockJournalEntryForm>({
    ...
    defaultValues: stockJournalForm.watch(entryPath) ?? {
  ...
  // Quantity is displayed with the stock unit's "No of Decimal Places"
  // (falls back to 2 when not set). Rate / discount / amount always use 2
  // decimal places.
  const selectedStockUnit =
    stockJournalEntryForm.getValues('stockUnit') ??
    stockUnits?.data?.data?.find(
      (su: StockUnit) =>
        su.id === stockJournalEntryForm.getValues('stockItem.stockUnitId'),
    )
  const qtyDecimalPlaces = useQuantityDecimals(
    selectedStockUnit?.noOfDecimalPlaces,
  )
  ...
  return (
    <Form {...stockJournalEntryForm}>
      <div ref={itemEntryRef} className="w-full grid grid-rows-1">
        <div
          className="grid grid-rows-1 grid-cols-[1fr_280px_130px_70px_70px_180px_120px] 
        ...
        >
          ...
          <div className="flex flex-row items-center gap-2 pr-1">
            {perRowMovementType && (
              <MovementToggle
                ...
                disabled={index === 0 && lockFirstRowMovementType}
              />
            )}
          ...
          <div className="grid grid-cols-2 items-start  text-right">
            <div className="pr-3">
              {Number(stockJournalEntryForm.watch('actualQuantity')!) > 0
                ? `${Number(
                    stockJournalEntryForm.watch('actualQuantity')!,
                  ).toFixed(
                    qtyDecimalPlaces,
                  )} ${selectedStockUnit?.code ?? ''}`.trim()
                : '-'}
            </div>
            {config.find((c) => c.key === 'show_actual_and_billing_quantity')
              ?.value ? (
              <div className="pr-3">
                {Number(stockJournalEntryForm.watch('billingQuantity')!) > 0
                  ? `${Number(
                      stockJournalEntryForm.watch('billingQuantity')!,
                    ).toFixed(
                      qtyDecimalPlaces,
                    )} ${selectedStockUnit?.code ?? ''}`.trim()
                  : '-'}
          ...
          <div className="text-right   pr-3 ">
            {stockJournalEntryForm.watch('billingQuantity')! > 0 &&
            stockJournalEntryForm.watch('rate')
              ? Number(stockJournalEntryForm.watch('rate')).toFixed(2)
              : '-'}{' '}
          </div>
          <div className="">
            <span className="font-bold"></span>
            {(stockJournalEntryForm.watch('billingQuantity')! > 0 &&
              stockJournalEntryForm.watch('rateUnit.code')) ??
              stockUnits?.data?.data?.find(
                (su: StockUnit) =>
                  su.id ===
                  stockJournalEntryForm.getValues('stockItem.stockUnitId'),
              )?.code}
          </div>
          <div className="text-right  pr-3 ">
            {Number(
              stockJournalEntryForm.watch('discountPercentage') ?? 0,
            ).toFixed(2)}
          </div>
          <div className="text-right  pr-3 ">
            {Number(stockJournalEntryForm.watch('amount') ?? 0).toFixed(2)}
          </div>
...
export const MovementToggle = ({
  ...
  const handleChange = (next: 'in' | 'out') => {
    ...
    const godownEntries =
      stockJournalEntryForm.getValues('stockJournalGodownEntries') ?? []
    godownEntries.forEach((_, i) => {
      stockJournalEntryForm.setValue(
        `stockJournalGodownEntries.${i}.movementType`,
        next,
        {
          shouldDirty: true,
        },
  ...
  const baseClass =
    'h-5 flex-1 rounded px-1.5 text-[10px] font-bold uppercase tracking-wide transition-colors disabled:cursor-not-allowed disabled:opacity-60'
