...
import type { ManufacturingJournalReportSchema } from '../data/schema'
...
interface DataTableProps {
  columns: ColumnDef<ManufacturingJournalReportSchema>[]
  data: ManufacturingJournalReportSchema[]
...
export function GridTable({
  ...
  const summary = useMemo(() => {
    ...
    for (const row of data) {
      const entries =
        row.stockJournal?.stockJournalEntries?.filter(Boolean) ?? []
    ...
    return { totalItems, consumptionQty, productionQty, grandTotal }
  }, [data])
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-4 px-2">
        <div className="text-sm text-muted-foreground">
          Total Manufacturing Journals:{' '}
          <span className="font-semibold">{totalVouchers}</span>
          {totalVouchers > 0 && (
            <span className="ml-2">
      ...
      <div className="rounded-md border">
        <Table>
          ...
          <TableBody>
              ...
              table.getRowModel().rows.map((row) => (
                <>
                  <TableRow
                    ...
                    onClick={() => {
                      const hasItems =
                        row.original.stockJournal?.stockJournalEntries
                          ?.length ?? 0 > 0
                  ...
                  {row.getIsExpanded() && (
                    ...
                    >
                      ...
                      >
                        <MovementDetails
                          stockJournalEntries={
                            row.original.stockJournal
                              ?.stockJournalEntries as any
                          }
                        />
                      </TableCell>
                    </TableRow>
            ...
            {data.length > 0 && (
              <TableRow
                className="border-t-2 border-primary/30 bg-primary/[0.04] font-semibold"
                style={{
                  display: 'grid',
                  gridTemplateColumns: gridTemplateCols,
                }}
              >
                <TableCell
                  className="flex items-center gap-1.5 text-primary py-3"
                ...
                <TableCell className="py-3">
                  <div className="flex items-center gap-2 text-xs text-muted-foreground">
                    ...
                    <span className="font-semibold text-foreground">
                      {formatQty(summary.consumptionQty)}
                    </span>
                ...
                <TableCell className="py-3">
                  <div className="flex items-center gap-2 text-xs text-muted-foreground">
                    ...
                    <span className="font-semibold text-foreground">
                      {formatQty(summary.productionQty)}
                    </span>
                    <span className="hidden sm:inline">produced</span>
                  </div>
                </TableCell>
                <TableCell className="py-3">
