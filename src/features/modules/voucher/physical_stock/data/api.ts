...
export async function storePhysicalStockService(payload: Record<string, unknown>) {
    return await postData(API_PATH, withPhysicalStockVoucherType(payload))
}

export async function updatePhysicalStockService(payload: Record<string, unknown>) {
    return await putData(`${API_PATH}/${payload.id}`, withPhysicalStockVoucherType(payload))
}
