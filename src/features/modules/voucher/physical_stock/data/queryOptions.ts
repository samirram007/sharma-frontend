...
export const PhysicalStockQueryOptions = (id?: number) => {
    return queryOptions({
        ...
        queryFn: () => (id ? fetchPhysicalStockByIdService(id) : fetchPhysicalStockService()),
        staleTime: 1000 * 60 * 5,
        retry: 1,
    })
}
...
export function usePhysicalStockVoucherMutation() {
    ...
    return useMutation({
        mutationFn: async (data: PhysicalStockVoucherMutationPayload) => {
            if (data.id) {
                return await updatePhysicalStockService(data as unknown as Record<string, unknown>)
            }
            return await storePhysicalStockService(data as unknown as Record<string, unknown>)
        },
        onSuccess: () => {
