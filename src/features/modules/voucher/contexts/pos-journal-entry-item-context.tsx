...
const PosJournalEntryItemContext =
  createContext<PosJournalEntryItemContextType | null>(null)

export const PosJournalEntryItemProvider = ({
  children,
}: {
  children: React.ReactNode
}) => {
  const addItemEntryButtonRef = React.useRef<HTMLButtonElement>(null)
  const [addItemButtonVisible, setAddItemButtonVisible] =
...
export const usePosJournalEntryItem = () => {
  const posJournalEntryItemContext = React.useContext(
    PosJournalEntryItemContext,
  )

  if (!posJournalEntryItemContext) {
    throw new Error(
      'usePosJournalEntryItem has to be used within <PosJournalEntryItemProvider>',
    )
  }
