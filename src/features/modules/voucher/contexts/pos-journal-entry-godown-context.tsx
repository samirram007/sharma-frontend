...
const PosJournalEntryGodownContext =
  createContext<PosJournalEntryGodownContextType | null>(null)

export const PosJournalEntryGodownProvider = ({
  children,
}: {
  children: React.ReactNode
}) => {
  const addGodownEntryButtonRef = React.useRef<HTMLButtonElement>(null)
  const [addGodownButtonVisible, setAddGodownButtonVisible] =
...
export const usePosJournalEntryGodown = () => {
  const posJournalEntryGodownContext = React.useContext(
    PosJournalEntryGodownContext,
  )

  if (!posJournalEntryGodownContext) {
    throw new Error(
      'usePosJournalEntryGodown has to be used within <PosJournalEntryGodownProvider>',
    )
  }
