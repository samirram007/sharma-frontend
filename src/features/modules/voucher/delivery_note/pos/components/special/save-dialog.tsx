...
const SaveDialog = ({ mainForm, isSaving, setSaving }: SaveDialogProps) => {
    ...
    useEffect(() => {
        ...
        timer = setTimeout(() => {
            ...
            if (!userFiscalYear) {
                newErrors.push("No active fiscal year assigned. Please assign one in settings.");
            } else if (userFiscalYear.fiscalYear?.status !== 'active') {
                newErrors.push(`The assigned fiscal year (${userFiscalYear.fiscalYear?.name}) is currently ${userFiscalYear.fiscalYear?.status}. It must be active to save vouchers.`);
            }
            ...
            if (userFiscalYear?.fiscalYear && voucherDate) {
                ...
                if (vDate < startDate || vDate > endDate) {
                    newErrors.push(`Voucher date (${vDate.toLocaleDateString()}) must be within the fiscal year period (${startDate.toLocaleDateString()} - ${endDate.toLocaleDateString()}).`);
                }
            }
