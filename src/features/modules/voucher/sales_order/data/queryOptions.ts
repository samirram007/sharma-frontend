...
export const salesOrderQueryOptions = (id?: number) => {
    return queryOptions({
        ...
        queryFn: () => (id ? fetchSalesOrderByIdService(id) : fetchSalesOrderService()),
        staleTime: 1000 * 60 * 5,
        retry: 1,
    })
}
...
export function useSalesOrderVoucherMutation() {
    ...
    return useMutation({
        mutationFn: async (data: SalesOrderVoucherMutationPayload) => {
            if (data.id) {
                return await updateSalesOrderService(data as unknown as Record<string, unknown>)
            }
            return await storeSalesOrderService(data as unknown as Record<string, unknown>)
        },
        onSuccess: () => {
