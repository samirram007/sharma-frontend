...
import { stockJournalSchema, type StockJournalEntryForm, type StockJournalForm } from '@/features/modules/voucher/data-schema/voucher-schema'
...
import salesOrderDefaultValues from '../data/data'
import { useSalesOrderVoucherMutation } from '../data/queryOptions'
import {
    SALES_ORDER_VOUCHER_TYPE_ID,
    salesOrderFormSchema,
    type SalesOrderVoucher,
    type SalesOrderVoucherForm,
} from '../data/schema'
import type { SalesOrderProps } from './contracts'
...
const buildEditValues = (row: SalesOrderVoucher): SalesOrderVoucherForm => ({
    ...
    voucherEntries:
        row.voucherEntries?.length > 0
            ...
            : salesOrderDefaultValues.voucherEntries,
...
function DateBox({ form, name }: { form: UseFormReturn<SalesOrderVoucherForm>; name: 'voucherDate' }) {
...
const Pos = ({ currentRow }: SalesOrderProps) => {
    const isEdit = Boolean(currentRow?.id)
    const { setMovementType } = usePos()
    const { mutate: saveSalesOrder, isPending: isSaving } = useSalesOrderVoucherMutation()

    const { data: customerLedgersResponse, isPending: isCustomersLoading } = useQuery({
        queryKey: ['accountLedgers', 'customer_ledgers'],
        queryFn: () => fetchPartyLedgerService('customer_ledgers'),
    })
    const { data: saleLedgersResponse, isPending: isSaleLedgersLoading } = useQuery({
    ...
    const customerItems = useMemo(
        () => customerLedgers.map((l) => ({ label: l.name ?? '', value: String(l.id ?? '') })),
        [customerLedgers]
    )
    const saleLedgerItems = useMemo(
        () => saleLedgers.map((l) => ({ label: l.name ?? '', value: String(l.id ?? '') })),
        [saleLedgers]
    )
    ...
    const [txLedgerStr, setTxLedgerStr] = useState<string>(
        currentRow?.transactionLedger?.id ? String(currentRow.transactionLedger.id) : ''
    )
    const [additionalEntries, setAdditionalEntries] = useState<AdditionalLedgerEntry[]>(() => {
    ...
    const mainForm = useForm<SalesOrderVoucherForm>({
        resolver: zodResolver(salesOrderFormSchema) as Resolver<SalesOrderVoucherForm>,
        defaultValues:
            isEdit && currentRow
                ? buildEditValues(currentRow)
                : { ...salesOrderDefaultValues, isEdit: false },
    })
    ...
    useEffect(() => {
        const subscription = stockJournalForm.watch((value) => {
            ...
            if (!isEqual(currentParent, value)) {
                mainForm.setValue('stockJournal', value as StockJournalForm, { shouldValidate: false })
            }
        })
        return () => subscription.unsubscribe()
    }, [mainForm, stockJournalForm])

    const watchedItems = useWatch({ control: stockJournalForm.control, name: 'stockJournalEntries' })
    const total = useMemo(
        () => (watchedItems ?? []).reduce((acc, entry) => acc + (Number(entry?.amount) || 0), 0),
        [watchedItems]
    )
    ...
    const recalcAmount = (index: number) => {
        ...
        stockJournalForm.setValue(`stockJournalEntries.${index}.amount`, amount, { shouldDirty: true })
    }
    ...
    const syncTrackingAmountAndQty = (index: number) => {
        ...
        stockJournalForm.setValue(`stockJournalEntries.${index}.stockJournalGodownEntries.0.actualQuantity`, Number(entry?.actualQuantity || 0), { shouldDirty: true })
        stockJournalForm.setValue(`stockJournalEntries.${index}.stockJournalGodownEntries.0.billingQuantity`, Number(entry?.billingQuantity || 0), { shouldDirty: true })
        stockJournalForm.setValue(`stockJournalEntries.${index}.stockJournalGodownEntries.0.rate`, Number(entry?.rate || 0), { shouldDirty: true })
        stockJournalForm.setValue(`stockJournalEntries.${index}.stockJournalGodownEntries.0.discountPercentage`, Number(entry?.discountPercentage || 0), { shouldDirty: true })
        stockJournalForm.setValue(`stockJournalEntries.${index}.stockJournalGodownEntries.0.amount`, Number(entry?.amount || 0), { shouldDirty: true })
    }
    ...
    const updateAdditionalLedgerRow = (rowId: string, patch: Partial<AdditionalLedgerEntry>) => {
        setAdditionalEntries((prev) =>
    ...
    useEffect(() => {
        const transactionLedgerId = Number(mainForm.getValues('transactionLedger.id') || 0)
        ...
        // Sales Order: Customer Dr, Sales A/c Cr
        const partyEntry = {
            id: null,
            voucherId: currentRow?.id ?? null,
            entryOrder: 1,
    ...
    const totalDebit = useMemo(
        () => watchedVoucherEntries.reduce((sum, entry) => sum + Number(entry?.debit || 0), 0),
        [watchedVoucherEntries]
    )
    const totalCredit = useMemo(
        () => watchedVoucherEntries.reduce((sum, entry) => sum + Number(entry?.credit || 0), 0),
        [watchedVoucherEntries]
    )
    const isBalanced = totalDebit > 0 && Math.abs(totalDebit - totalCredit) < 0.0001

    useEffect(() => { setMovementType?.('out') }, [setMovementType])
    ...
    const onSubmit = (values: SalesOrderVoucherForm) => {
        ...
        saveSalesOrder(
            {
                ...
                voucherEntries: coercedEntries as Array<{ id?: number | null; voucherId?: number | null; entryOrder: number; accountLedgerId: number; debit: number; credit: number; remarks?: string | null }>,
            },
            {
                onSuccess: () => {
                    if (!isEdit) {
                        mainForm.reset({ ...salesOrderDefaultValues, isEdit: false })
                ...
                >
                    <div className='border-b border-slate-300 px-3 py-2 space-y-2'>
                        <div className='flex items-center justify-between'>
                            <div>
                                <p className='text-xs text-slate-500'>Voucher Type ID: 5002</p>
                                <h2 className='text-base font-semibold text-slate-800'>Sales Order</h2>
                            </div>
                            <div className={`rounded px-2 py-1 text-xs font-semibold ${isBalanced ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700'}`}>
                        ...
                        <div className='grid gap-2 lg:grid-cols-2'>
                            <div>
                                <div className='mb-1 text-[11px] font-semibold uppercase text-slate-500'>Party A/c (Customer)</div>
                                <SelectDropdown
                                    defaultValue={partyLedgerStr}
                                    onValueChange={(val) => {
                                        setPartyLedgerStr(val)
                                        const ledger = customerLedgers.find((l) => String(l.id) === val)
                                        if (ledger) {
                            ...
                            <div>
                                <div className='mb-1 text-[11px] font-semibold uppercase text-slate-500'>Sales A/c</div>
                                <SelectDropdown
                        ...
                        {canShowItems ? (
                            <Form {...stockJournalForm}>
                                <table className='w-full border-collapse text-[13px]'>
                                    <thead>
                                        <tr className='border-b border-slate-300 bg-slate-50 text-xs uppercase tracking-wide text-slate-500'>
                                            ...
                                            <th className='w-24 px-2 py-2 text-right font-semibold'>Disc %</th>
                                            <th className='w-32 px-2 py-2 text-right font-semibold'>Amount</th>
                                            <th className='w-20 px-2 py-2 text-center font-semibold'>Action</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {itemFields.map((field, index) => {
                                            const entry = watchedItems?.[index]
                                            const selectedItem = stockItems.find((s) => s.id === Number(entry?.stockItemId || 0))
                                            const unit = stockUnits.find((u) => u.id === Number(selectedItem?.stockUnitId || entry?.stockUnitId || 0))
                                            const trackingRequired = isTrackingRequired(entry?.stockItemId)
                                                ...
                                                <Fragment key={field.id}>
                                                    <tr className='border-b border-slate-200 hover:bg-slate-50'>
                                                        ...
                                                        <td className='px-2 py-1'>
                                                            <SelectDropdown
                                                                defaultValue={entry?.stockItemId ? String(entry.stockItemId) : ''}
                                                                onValueChange={(val) => {
                                                                    const item = stockItems.find((s) => String(s.id) === val)
                                                                    stockJournalForm.setValue(`stockJournalEntries.${index}.stockItemId`, Number(val), { shouldDirty: true })
                                                                    stockJournalForm.setValue(`stockJournalEntries.${index}.stockItem`, (item ?? null) as StockJournalEntryForm['stockItem'], { shouldDirty: true })
                                                                    stockJournalForm.setValue(`stockJournalEntries.${index}.stockUnitId`, item?.stockUnitId ?? null, { shouldDirty: true })
                                                        ...
                                                        <td className='px-2 py-1'>
                                                            <input
                                                                ...
                                                                onChange={(e) => {
                                                                    stockJournalForm.setValue(`stockJournalEntries.${index}.billingQuantity`, Number(e.target.value), { shouldDirty: true })
                                                                    stockJournalForm.setValue(`stockJournalEntries.${index}.actualQuantity`, Number(e.target.value), { shouldDirty: true })
                                                                    recalcAmount(index)
                                                                    if (trackingRequired) {
                                                                        ensureTrackingEntry(index)
                                                        ...
                                                        <td className='px-2 py-1 text-slate-500 text-xs'>{unit?.code ?? '-'}</td>
                                                        <td className='px-2 py-1'>
                                                            <input
                                                                type='number'
                                                                min='0'
                                                                ...
                                                                onChange={(e) => {
                                                                    stockJournalForm.setValue(`stockJournalEntries.${index}.rate`, Number(e.target.value), { shouldDirty: true })
                                                                    recalcAmount(index)
                                                                    if (trackingRequired) {
                                                                        ensureTrackingEntry(index)
                                                                        syncTrackingAmountAndQty(index)
                                                        ...
                                                        <td className='px-2 py-1'>
                                                            <input
                                                                ...
                                                                onChange={(e) => {
                                                                    stockJournalForm.setValue(`stockJournalEntries.${index}.discountPercentage`, Number(e.target.value), { shouldDirty: true })
                                                                    recalcAmount(index)
                                                                    if (trackingRequired) {
                                                                        ensureTrackingEntry(index)
                                                                        syncTrackingAmountAndQty(index)
                                                        ...
                                                        <tr className='border-b border-slate-200 bg-slate-50/60'>
                                                            <td colSpan={8} className='px-2 py-2'>
                                                                ...
                                                                <div className='grid gap-2 lg:grid-cols-[minmax(220px,1fr)_minmax(180px,1fr)_minmax(180px,1fr)_minmax(180px,1fr)_minmax(180px,1fr)]'>
                                                                    <SelectDropdown
                                                                        defaultValue={trackingRow?.godownId ? String(trackingRow.godownId) : ''}
                                                                        onValueChange={(value) => {
                                                                            ensureTrackingEntry(index)
                                                                            stockJournalForm.setValue(
                                                                    ...
                                                                    {Boolean(selectedItem?.isMaintainBatch) && (
                                                                        <input
                                                                            ...
                                                                            onChange={(event) => {
                                                                                ensureTrackingEntry(index)
                                                                                stockJournalForm.setValue(`stockJournalEntries.${index}.stockJournalGodownEntries.0.batchNo`, event.target.value, { shouldDirty: true })
                                                                            }}
                                                                        />
                                                                    )}
                                                                    {Boolean(selectedItem?.isMaintainSerial) && (
                                                                        <input
                                                                            ...
                                                                            onChange={(event) => {
                                                                                ensureTrackingEntry(index)
                                                                                stockJournalForm.setValue(`stockJournalEntries.${index}.stockJournalGodownEntries.0.serialNo`, event.target.value, { shouldDirty: true })
                                                                            }}
                                                                        />
                                                                    )}
                                                                    {Boolean(selectedItem?.trackManufacturingDate) && (
                                                                        <input
                                                                            type='date'
                                                                            className='h-8 rounded border border-slate-300 bg-white px-2 text-sm outline-none focus:border-slate-400'
                                                                            value={trackingRow?.mfgDate ? new Date(trackingRow.mfgDate).toISOString().slice(0, 10) : ''}
                                                                            onChange={(event) => {
                                                                                ensureTrackingEntry(index)
                                                                                stockJournalForm.setValue(`stockJournalEntries.${index}.stockJournalGodownEntries.0.mfgDate`, event.target.value ? new Date(event.target.value) : null, { shouldDirty: true })
                                                                            }}
                                                                        />
                                                                    )}
                                                                    {Boolean(selectedItem?.useExpiryDate) && (
                                                                        <input
                                                                            type='date'
                                                                            className='h-8 rounded border border-slate-300 bg-white px-2 text-sm outline-none focus:border-slate-400'
                                                                            value={trackingRow?.expiryDate ? new Date(trackingRow.expiryDate).toISOString().slice(0, 10) : ''}
                                                                            onChange={(event) => {
                                                                                ensureTrackingEntry(index)
                                                                                stockJournalForm.setValue(`stockJournalEntries.${index}.stockJournalGodownEntries.0.expiryDate`, event.target.value ? new Date(event.target.value) : null, { shouldDirty: true })
                                                                            }}
                                                                        />
                                                                    )}
                                                                </div>
                                ...
                                <div className='px-3 py-2'>
                                    <Button type='button' variant='outline' size='sm' className='h-8 text-xs' onClick={addItemRow}>
                                        + Add Item
                                    </Button>
                                </div>
                                ...
                                <div className='border-t border-slate-200 px-3 py-2'>
                                    <div className='mb-2 flex items-center justify-between'>
                                        <div className='text-xs font-semibold uppercase text-slate-500'>Additional Ledger Entries</div>
                                        <Button type='button' variant='outline' size='sm' className='h-8 text-xs' onClick={addAdditionalLedgerRow}>
                                            + Add Ledger
                                        </Button>
                                    </div>
                                        ...
                                        <table className='w-full border-collapse text-[13px]'>
                                            <thead>
                                                <tr className='text-xs uppercase text-slate-400'>
                                                    ...
                                                    <th className='w-24 pb-1 text-left font-semibold'>Dr/Cr</th>
                                                    <th className='w-28 pb-1 text-right font-semibold'>Amount</th>
                                                    <th className='pb-1 text-left font-semibold pl-2'>Remarks</th>
                                                    <th className='w-16' />
                                                </tr>
                                            </thead>
                                            <tbody>
                                                {additionalEntries.map((row) => (
                                                    <tr key={row.id} className='border-t border-slate-100'>
                                                        <td className='py-1 pr-2'>
                                                            <SelectDropdown
                                                                defaultValue={row.accountLedgerId ? String(row.accountLedgerId) : ''}
                                                                onValueChange={(val) => updateAdditionalLedgerRow(row.id, { accountLedgerId: Number(val) })}
                                                        ...
                                                        <td className='py-1 pr-2'>
                                                            <select
                                                                ...
                                                                value={row.entryType}
                                                                onChange={(e) => updateAdditionalLedgerRow(row.id, { entryType: e.target.value as 'debit' | 'credit' })}
                                                            >
                                                        ...
                                                        <td className='py-1 pr-2'>
                                                            <input
                                                                ...
                                                                className='h-8 w-full rounded border border-slate-300 bg-white px-2 text-right text-sm outline-none focus:border-slate-400'
                                                                value={row.amount}
                                                                onChange={(e) => updateAdditionalLedgerRow(row.id, { amount: Number(e.target.value) })}
                                                            />
                                                        </td>
                                                        <td className='py-1 pr-2'>
                                                            <input
                                                                ...
                                                                value={row.remarks}
                                                                onChange={(e) => updateAdditionalLedgerRow(row.id, { remarks: e.target.value })}
                                                            />
                                                        </td>
                    ...
                    <div className='border-t border-slate-300 px-3 py-2'>
                        <div className='mb-2'>
                            <div className='mb-1 text-[11px] font-semibold uppercase text-slate-500'>Narration</div>
                            <textarea
                        ...
                        <div className='flex items-center justify-between'>
                            <div className='flex gap-4 text-xs text-slate-600'>
                                <span>Item Total: <strong>{total.toFixed(2)}</strong></span>
                                <span className='text-emerald-700'>Dr: <strong>{totalDebit.toFixed(2)}</strong></span>
                                <span className='text-rose-700'>Cr: <strong>{totalCredit.toFixed(2)}</strong></span>
                                <span className={`font-semibold ${isBalanced ? 'text-emerald-700' : 'text-amber-600'}`}>
                                    {isBalanced ? 'Balanced' : 'Not Balanced'}
                                </span>
                            </div>
