...
export const journalVoucherQueryOptions = () => {
    return queryOptions({
        ...
        queryFn: async () => {
            ...
            return parsed.filter((voucher) => voucher.voucherTypeId === JOURNAL_VOUCHER_TYPE_ID)
        },
        staleTime: 1000 * 60,
        retry: 1,
    })
...
export function useJournalVoucherMutation() {
    ...
    return useMutation({
        mutationFn: async (data: JournalVoucherMutationPayload) => {
            if (data.id) {
                return await updateJournalService(data as unknown as Record<string, unknown>)
            }
            return await storeJournalService(data as unknown as Record<string, unknown>)
        },
        onSuccess: () => {
