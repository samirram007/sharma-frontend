...
import journalDefaultValues from '../data/data'
import { useJournalVoucherMutation } from '../data/queryOptions'
import {
  JOURNAL_VOUCHER_TYPE_ID,
  journalFormSchema,
  type JournalVoucher,
  type JournalVoucherForm,
} from '../data/schema'
import type { JournalProps } from './contracts'

const buildEditValues = (row: JournalVoucher): JournalVoucherForm => ({
  ...
  voucherEntries:
    row.voucherEntries?.length > 0
      ...
      : journalDefaultValues.voucherEntries,
...
const getEntryType = (entry: JournalVoucherForm['voucherEntries'][number]) =>
  Number(entry?.credit || 0) > 0 ? 'credit' : 'debit'

const Pos = ({ currentRow }: JournalProps) => {
  ...
  const { mutate: saveJournal, isPending: isSaving } =
    useJournalVoucherMutation()
  ...
  const mainForm = useForm<JournalVoucherForm>({
    resolver: zodResolver(journalFormSchema) as Resolver<JournalVoucherForm>,
    defaultValues:
      isEdit && currentRow
        ? buildEditValues(currentRow)
        : { ...journalDefaultValues, isEdit: false },
  })
  ...
  const handleEntryTypeChange = (
    ...
    if (nextType === 'debit') {
      ...
      mainForm.setValue(`voucherEntries.${index}.debit`, 0, {
        shouldDirty: true,
      })
  ...
  const onSubmit = (values: JournalVoucherForm) => {
    ...
    saveJournal(
      ...
      },
      {
        onSuccess: () => {
          if (!isEdit) {
            mainForm.reset({ ...journalDefaultValues, isEdit: false })
          }
        },
      },
    )
        ...
        >
          <div className="border-b border-slate-300 px-3 py-2">
            <div className="mb-2 flex items-center justify-between">
              <div>
                <p className="text-xs text-slate-500">Voucher Type ID: 1004</p>
...
type DateBoxProps = {
  form: UseFormReturn<JournalVoucherForm>
  name: 'voucherDate'
}
