...
export const DebitNoteQueryOptions = (id?: number) => {
    return queryOptions({
        ...
        queryFn: () => (id ? fetchDebitNoteByIdService(id) : fetchDebitNoteService()),
        staleTime: 1000 * 60 * 5,
        retry: 1,
    })
}
...
export function useDebitNoteVoucherMutation() {
    ...
    return useMutation({
        mutationFn: async (data: DebitNoteVoucherMutationPayload) => {
            if (data.id) {
                return await updateDebitNoteService(data as unknown as Record<string, unknown>)
            }
            return await storeDebitNoteService(data as unknown as Record<string, unknown>)
        },
        onSuccess: () => {
