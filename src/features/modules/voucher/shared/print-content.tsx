...
export const FreightPrintContent = (printData: PrintContentData) => {
  const dispatchDetail =
    ...
    printData.voucherReferences?.find((vr) => vr.referenceVoucher?.voucherTypeId === 2001)
  ...
  const totalFare = Number(printData.amount) || Number(dispatchDetail?.totalFare) || 0
    ...
    <>
      <div className="x-header">
        ...
        <div className="grid grid-cols-2 gap-2 mt-1">
          <div className="flex gap-2">
            <span>Transport Name:</span> <span className="underline decoration-dotted underline-offset-4 "> {dispatchDetail?.carrierName}</span>
          </div>
          <div className="flex justify-end gap-2">
            <span>Truck No:</span> <span className="underline decoration-dotted underline-offset-4 "> {dispatchDetail?.motorVehicleNo}</span>
          </div>
          {weight > 0 && (
            <div className="flex gap-2">
              <span>Weight:</span> <span className="underline decoration-dotted underline-offset-4 "> {weight.toFixed(weightDecimals)}{weightUnitCode ? ` ${weightUnitCode}` : ''}</span>
            </div>
          )}
          {rate > 0 && (
            <div className="flex justify-end gap-2">
              <span>Rate:</span> <span className="underline decoration-dotted underline-offset-4 "> {rate.toFixed(2)}</span>
            </div>
          )}
        </div>
        {dispatchDetail?.billingPreference && (
          <div className="grid grid-cols-2 mt-1">
            <div className="flex gap-2">
              <span>Billing:</span> <span className="capitalize underline decoration-dotted underline-offset-4 font-semibold">{dispatchDetail.billingPreference}</span>
            </div>
          </div>
        )}
      </div>
        ...
        <div className="grid  gap-4">
          <div className="grid grid-cols-[auto_1fr] items-start  gap-4">
            ...
            <div className="border-b-2 border-y-zinc-400 border-dotted text-xl">
              <span className="px-4"></span>{printData?.partyLedger?.name}
            </div>
          </div>
          ...
          <div>
            <div className="border-b-2 border-zinc-900 border-dotted italic"><span>Rupees </span> {numberToWords(printData?.amount!)}{discount > 0 && <> (after discount of {numberToWords(Math.round(discount))})</>}</div>
          </div>
  ...
  return (
    <>
      <div className="x-header">
        ...
        <div className="grid grid-cols-2">
          ...
          <div className="flex flex-col items-end justify-end">
            ...
            {weight > 0 && (
              <div className="">
                Weight: <span className="underline decoration-dotted underline-offset-4 "> {weight.toFixed(weightDecimals)}{weightUnitCode ? ` ${weightUnitCode}` : ''}</span>
              </div>
            )}
            {rate > 0 && (
              <div className="">
                Rate: <span className="underline decoration-dotted underline-offset-4 "> {rate.toFixed(2)}</span>
              </div>
            )}
      ...
      <div className="x-body mt-2 ">
        ...
        <div className="border-b-2 border-zinc-900 border-dotted italic"><span>Rupees </span> {numberToWords(totalFare)}</div>
      </div>
      <div className="x-footer h-12 mt-12 grid grid-cols-2">
        <div className="flex flex-row items-end gap-6">
          <div className="w-12 h-12 border-2 border-zinc-900"></div>
