...
import { useConversionJournalVoucherMutation } from '../../../data/queryOptions'
...
const SaveDialog = ({ mainForm, isSaving, setSaving }: SaveDialogProps) => {
  ...
  const { mutate: saveConversionJournal, isPending } =
    useConversionJournalVoucherMutation()
  ...
  const handleSaving = () => {
    saveConversionJournal(mainForm.getValues())
  }
  ...
  useEffect(() => {
    ...
    timer = setTimeout(() => {
      ...
      if (!hasInEntry) {
        newErrors.push(
          'At least one IN entry (output of conversion) is required.',
        )
      }
      if (!hasOutEntry) {
      ...
      >
        ...
        <DialogContent className="sm:max-w-5xl">
          <DialogHeader className="text-left border-b-2 pb-2">
            <DialogTitle>Conversion Journal </DialogTitle>
            <DialogDescription>
              Please wait while we are saving the conversion journal.
            </DialogDescription>
          </DialogHeader>
