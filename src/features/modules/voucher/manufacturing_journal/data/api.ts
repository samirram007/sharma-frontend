...
const withManufacturingJournalVoucherType = (
...
export async function storeManufacturingJournalService(
  payload: Record<string, unknown>,
) {
  return await postData(API_PATH, withManufacturingJournalVoucherType(payload))
}

export async function updateManufacturingJournalService(
  payload: Record<string, unknown>,
) {
  return await putData(
    `${API_PATH}/${payload.id}`,
    withManufacturingJournalVoucherType(payload),
  )
}
