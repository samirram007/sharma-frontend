import { Route as ManufacturingJournalReportRoute } from '@/routes/_protected/reports/manufacturing_journal_report/index'
import {
  queryOptions,
  useMutation,
  useQueryClient,
...
export const ManufacturingJournalVoucherQueryOptions = () => {
  return queryOptions({
    ...
    queryFn: async () => {
      ...
      const parsed = ManufacturingJournalVoucherListSchema.parse(
        response?.data ?? [],
      )
      return parsed.filter(
        (voucher) =>
          voucher.voucherTypeId === MANUFACTURING_JOURNAL_VOUCHER_TYPE_ID,
      )
    },
    staleTime: 1000 * 60,
    retry: 1,
...
export const ManufacturingJournalQueryOptions = (id?: number) => {
  return queryOptions({
    ...
    queryFn: () =>
      id
        ? fetchManufacturingJournalByIdService(id)
        : fetchManufacturingJournalService(),
    staleTime: 1000 * 60 * 5,
    retry: 1,
  })
...
type ManufacturingJournalVoucherMutationPayload =
  ManufacturingJournalVoucherForm & { id?: number }
...
export function useManufacturingJournalVoucherMutation() {
  ...
  return useMutation({
    mutationFn: async (data: ManufacturingJournalVoucherMutationPayload) => {
      if (data.id) {
        return await updateManufacturingJournalService(
          data as unknown as Record<string, unknown>,
        )
      }
      return await storeManufacturingJournalService(
        data as unknown as Record<string, unknown>,
      )
    },
    ...
    onError: (error) => {
      ...
      toast.error(
        getServerErrorMessage(
          error,
          'Failed to save manufacturing journal voucher.',
        ),
      )
    },
  })
