...
import { buildPaymentVoucherEntries, calculateTotalDebit } from './payment-payload'
...
const Pos = ({ currentRow }: PaymentProps) => {
    ...
    const { mutate: savePayment, isPending: isSaving } = usePaymentVoucherMutation()
    ...
    const initialCreditLedgerId = useMemo(() => {
        ...
        return creditEntry?.accountLedgerId ? String(creditEntry.accountLedgerId) : ''
    }, [currentRow])
    ...
    const totalDebit = useMemo(() => calculateTotalDebit(watchedEntries), [watchedEntries])

    const hasDebitEntries = useMemo(
        () =>
            watchedEntries.some(
                (entry) => Number(entry?.accountLedgerId || 0) > 0 && Number(entry?.debit || 0) > 0
            ),
        [watchedEntries]
    )

    const isBalanced = Boolean(creditLedgerId) && totalDebit > 0 && hasDebitEntries
                ...
                >
                    <div className='border-b border-slate-300 px-3 py-2'>
                        <div className='mb-2 flex items-center justify-between'>
                            <div>
                                <h2 className='text-base font-semibold text-slate-800'>Payment Voucher</h2>
                                <p className='text-xs text-slate-500'>Header credit account with debit line entries</p>
                            </div>
                            <div
                        ...
                        <div className='space-y-2'>
                            ...
                            <div className='grid gap-2 lg:grid-cols-[180px_180px_minmax(280px,1fr)_auto_auto]'>
                                <div className='max-w-md lg:col-start-1 lg:col-span-2'>
                                    <div className='mb-1 text-[11px] font-semibold uppercase text-slate-500'>Header Credit Account</div>
                                    <SelectDropdown
                                    ...
                                    return (
                                        <tr key={field.id} className='border-b border-slate-200'>
                                            <td className='px-2 py-2 text-center'>
                                                <span className='inline-flex rounded bg-slate-100 px-2 py-1 text-xs font-semibold text-slate-600'>Debit</span>
                                            </td>
