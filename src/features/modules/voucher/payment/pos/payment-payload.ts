...
export interface PaymentCreditLedgerInput {
    id: number
    name: string
    code: string
...
export const calculateTotalDebit = (entries: PaymentEntryInput[]) =>
    entries.reduce((sum, entry) => sum + Number(entry?.debit || 0), 0)

export const buildPaymentVoucherEntries = (params: {
    entries: PaymentEntryInput[]
    selectedCreditLedger: PaymentCreditLedgerInput
