...
export default function ClosingStock({
  ...
  const exportRows = useMemo(
    () =>
      view === 'batch'
        ? flattenBatchWise(batchWiseItems)
        : flattenGodownWise(data.items),
  ...
  const downloadCsv = useCallback(
    (headers: string[], flatRows: string[][], tab: string) => {
      ...
      const csv =
        bom +
        [headers.join(','), ...flatRows.map((row) => row.join(','))].join('\n')
  ...
  const downloadJson = useCallback(
    (exportData: unknown, tab: string) => {
      downloadBlob(
        JSON.stringify(exportData, null, 2),
        `closing-stock-${tab}.json`,
        'application/json',
      )
    },
    [downloadBlob],
      ...
      <span className="min-w-0 text-muted-foreground">
        {isFrozen ? (
          <>
            ...
            dated {data.closingDate ? date_format(data.closingDate) : '—'} —
            stock frozen at fiscal year end.
          </>
        ) : (
          <>
            No closing stock journal (CLSSK) found for this fiscal year —
            closing stock is computed live from stock movements
            {data.asOfDate ? (
              <>
                {' '}
                as of{' '}
                <span className="font-semibold text-foreground">
                  {date_format(data.asOfDate)}
                </span>
              </>
            ) : null}
            .
...
const ReportHeader = () => (
  <div className="grid grid-cols-[1fr_3fr] border-2 bg-gray-100 text-center font-bold dark:bg-zinc-900">
    <div className="flex items-center border-r-2 px-2 text-left">
      PARTICULARS
    </div>
  ...
  <div className="text-sm italic text-muted-foreground">
    <div className="grid grid-cols-[1fr_3fr] text-center">
      ...
      <div className="grid grid-cols-3 tabular-nums">
        ...
        <div className="border-l-2 px-2 text-right">
          {formatAmount(godown.closingAmount)}
        </div>
      </div>
    </div>
        ...
        {godown.batchDetails.map((batch, batchIndex) => (
          ...
          >
            <div className="flex min-w-0 flex-wrap items-center gap-x-2 px-2 pl-12 text-left not-italic">
              ...
              {batch.expiryDate && (
                <span>Exp: {date_format(batch.expiryDate)}</span>
              )}
            </div>
            <div className="grid grid-cols-3 tabular-nums">
              <div className="border-l-2 px-2 text-right">
...
const ReportFooter = ({ data }: { data: ClosingStockSchema }) => {
  const units = Array.from(
    new Set(data.items.map((item) => item.unitCode).filter(Boolean)),
  )
  const unitCode = units.length === 1 ? units[0] : '~'
  const decimals =
    units.length === 1 ? (data.items[0]?.noOfDecimalPlaces ?? 2) : 2
  ...
  return (
    <div className="grid grid-cols-[1fr_3fr] border-2 border-t-0 bg-gray-100 text-center font-bold dark:bg-zinc-900">
      <div className="flex items-center justify-between gap-2 px-2 text-left">
        <span className="font-mono text-xs font-normal italic">
 ...
 */
function buildBatchWiseItems(
  items: ClosingStockItemSchema[],
): BatchWiseItemSchema[] {
 ...
 */
function flattenGodownWise(
  items: ClosingStockItemSchema[],
): ClosingStockExportRow[] {
 ...
 */
function flattenBatchWise(
  items: BatchWiseItemSchema[],
): ClosingStockExportRow[] {
  const rows: ClosingStockExportRow[] = []
...
function formatClosingStockExportRow(
  row: ClosingStockExportRow,
...
function computeClosingStockTotals(
  rows: ClosingStockExportRow[],
): Record<string, string> {
