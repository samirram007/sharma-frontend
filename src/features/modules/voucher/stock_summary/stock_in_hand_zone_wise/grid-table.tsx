      ...
      <div className="border-2 border-border min-h-full">
        {table.getRowModel().rows.map((zone: any, index: number) => (
          <div key={index} className="grid grid-rows-1 gap-0">
            ...
            <div>
              {zone.original.godownDetails.map(
  ...
  useEffect(() => {
    ...
    table.getRowModel().rows.forEach((row: any) => {
      ...
      if (item.godownDetails.length > 0) {
        ...
        noOfDecimalPlaces.add(
          item.godownDetails[0]?.itemDetails[0]?.noOfDecimalPlaces,
        )
      }
    })
