...
export const TransactionLedgerCombobox = ({
  form,
  transactionLedgers,
}: Props) => {
  ...
  const [value, setValue] = React.useState(
    form.getValues('transactionLedger.id')?.toString(),
  )

  const handleSelect = (value: string) => {
    form.setValue(
      'transactionLedger.id',
      transactionLedgers.find(
        (transactionLedger) => transactionLedger.id === Number(value),
      )?.id!,
    )
    setValue(value)
    setOpen(false)
  ...
  const handleBlur = (e: React.FocusEvent<HTMLButtonElement>) => {
    ...
    if (!form.getValues('transactionLedger.id')) setOpen(true)
  }
  const frameworks = transactionLedgers?.map(
