...
import {
  ...
  FreightCalculator,
  DateBox,
} from '@/features/modules/voucher/freight/components/shared/freight-calculator'
...
const VoucherDispatchDetail = () => {
  ...
  const [viewMode, setViewMode] = useLocalStorage<'tabbed' | 'single'>(
    'dispatchViewMode',
    'tabbed',
  )
  ...
  const sectionVisibility = (key: string) => ({
    checked: !!config.find((c) => c.key === key)?.value,
    onCheckedChange: (checked: boolean) => updateConfig(key, checked),
  })
  ...
  const handleOnClick = () => {
    form.setValue(
      ...
      voucherDisplayDispatchForm.getValues(),
    )
      ...
      >
        <DialogTrigger asChild>
          <Button
            variant="outline"
            size="sm"
            className="py-1! px-2! focus:bg-black focus:text-white"
          >
            Receipt Details
          </Button>
        </DialogTrigger>
        <DialogContent className="sm:max-w-4xl max-h-[85vh] overflow-hidden flex flex-col">
          <DialogHeader className="border-b border-slate-200 pb-3 dark:border-slate-700">
                  ...
                  <PopoverContent align="end" className="w-56 p-3">
                    ...
                    <div className="flex flex-col gap-2.5">
                      <label className="flex items-center justify-between">
                        ...
                        <Switch
                          checked={
                            !!config.find((c) => c.key === 'order_details')
                              ?.value
                          }
                          onCheckedChange={(checked) =>
                            updateConfig('order_details', checked)
                          }
                        />
                      </label>
                      <label className="flex items-center justify-between">
                        ...
                        <Switch
                          checked={
                            !!config.find((c) => c.key === 'transport_details')
                              ?.value
                          }
                          onCheckedChange={(checked) =>
                            updateConfig('transport_details', checked)
                          }
                        />
                      </label>
                      <label className="flex items-center justify-between">
                        ...
                        <Switch
                          checked={
                            !!config.find((c) => c.key === 'receipt_details')
                              ?.value
                          }
                          onCheckedChange={(checked) =>
                            updateConfig('receipt_details', checked)
                          }
                        />
                      </label>
                      <label className="flex items-center justify-between">
                        ...
                        <Switch
                          checked={
                            !!config.find((c) => c.key === 'freight_details')
                              ?.value
                          }
                          onCheckedChange={(checked) =>
                            updateConfig('freight_details', checked)
                          }
                        />
                      </label>
                    </div>
            ...
            <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
              Fill in the receipt and dispatch details below, then click Save to
              apply.
            </p>
          </DialogHeader>
          <div
              ...
              {viewMode === 'tabbed' ? (
                <Tabs defaultValue="dispatch-info" className="w-full">
                  <TabsList className="mb-4 w-full justify-start gap-1 rounded-lg bg-slate-100 p-1 dark:bg-slate-800/60">
                    <TabsTrigger
                      value="dispatch-info"
                      className="flex items-center gap-1.5 rounded-md px-3 py-1.5 text-xs font-medium data-[state=active]:bg-white data-[state=active]:shadow-sm dark:data-[state=active]:bg-slate-800"
                    >
                  ...
                  <TabsContent value="transport" className="mt-0 space-y-5">
                    <SectionVisibilityHeader
                      ...
                      {...sectionVisibility('transport_details')}
                    />
                    {config.find((c) => c.key === 'transport_details')
                      ?.value && (
                  ...
                  <TabsContent value="freight" className="mt-0 space-y-5">
                    <SectionVisibilityHeader
                      ...
                      label="Show Freight Calculator"
                      {...sectionVisibility('freight_details')}
                    />
...
type SectionVisibilityHeaderProps = {
  title: string
  label: string
  checked: boolean
...
const SectionVisibilityHeader = ({
  title,
  label,
  checked,
  onCheckedChange,
}: SectionVisibilityHeaderProps) => {
  return (
