...
export const GstRegistrationTypeCombobox = ({ form }: Props) => {
  const { data: gstRegistrationTypes } = useSuspenseQuery(
    gstRegistrationTypeQueryOptions(),
  )
  const [open, setOpen] = React.useState(false)
  const [selectedId] = React.useState(
    form.getValues('gstRegistrationTypeId')?.toString(),
  )
  ...
  const selected = frameworks.find(
    (o: { label: string; value: string }) => o.value === selectedId,
  )
  // console.log("SELECTED GODOWN: ", selectedId, selected)
  const selectedLabel = selected
    ? (selected?.label?.toString() ?? 'Select gst_registration_type')
    : 'Select gst_registration_type'
