...
export default function FreightTransporterItemWise({ data, title = 'Freight (Transporter Item Wise)' }: TransporterItemWiseProps) {
  return (
    <>
      {!data || data.length === 0 ? (
...
const COLUMN_GRID = 'grid-cols-[1.1fr_0.8fr_1.2fr_1.5fr_1fr_1fr_0.65fr_0.75fr_0.7fr_0.55fr]'

const ReportView = ({ data, title }: { data: Array<TransporterItemWiseItem>; title: string }) => {
  ...
  const filteredData = useMemo(() => {
    ...
    if (transporterFilter.length > 0) {
      result = result.filter((t) => transporterFilter.includes(t.transporterName))
    }
    ...
    <div className='w-full min-h-full grid grid-rows-[auto_auto_1fr]'>
      ...
      <div>
        {showChart && (
          <div className='px-2 py-3 border-b'>
            <TransporterChart data={data} chartTitle='Transporter Wise Total Amounts' />
          </div>
        )}
      </div>
      ...
      <div className='min-h-full overflow-auto space-y-4 p-3'>
        {filteredData.map((transporter, index) => (
          <TransporterSection key={transporter.transporterName ?? index} transporter={transporter} />
        ))}
          ...
          <div className='sticky bottom-0 bg-gradient-to-r from-gray-800 to-gray-900 text-white rounded-lg shadow-lg border border-gray-700'>
            <div className='flex items-center justify-between px-4 py-2 text-[13px] font-bold'>
              <div className='flex items-center gap-2'>
                ...
                <span className='tracking-widest uppercase text-amber-300'>Grand Total</span>
              </div>
              <span className='text-gray-300 text-xs'>
                {filteredData.length} transporter{filteredData.length !== 1 ? 's' : ''} · {grandTotal.totalVouchers} voucher{grandTotal.totalVouchers !== 1 ? 's' : ''}
              </span>
              <span className='text-gray-300 text-xs'>
                {grandTotal.totalEntries ?? 0} line{grandTotal.totalEntries !== 1 ? 's' : ''}
              </span>
              <span className='text-amber-300 text-sm'>₹{(grandTotal.totalAmount ?? 0).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
            </div>
          </div>
        )}
      </div>
...
const TransporterSection = ({ transporter }: { transporter: TransporterItemWiseItem }) => {
  // Pagination for entries
  const [entryPage, setEntryPage] = useState(0)
  const [entryPageSize, setEntryPageSize] = useState(20)
  const totalEntryPages = Math.max(1, Math.ceil((transporter.entries?.length ?? 0) / entryPageSize))
  const safePage = Math.min(entryPage, totalEntryPages - 1)
  const pageEntries = (transporter.entries ?? []).slice(safePage * entryPageSize, (safePage + 1) * entryPageSize)
  ...
  const allExpanded = useMemo(() => {
    ...
    return voucherGroups.every((g) => expandedVouchers[g.voucherNo] ?? g.items.length <= 1)
  ...
  return (
    <div className='rounded-lg border border-gray-200 bg-white shadow-sm transition-shadow hover:shadow-md overflow-hidden'>
      ...
      <div className='rounded-t-lg bg-gradient-to-r from-blue-600 to-blue-700 text-white shadow-md'>
        <div className='flex items-center justify-between px-3 py-2'>
          <div className='flex items-center gap-2 min-w-0'>
            ...
            <span className='text-sm font-bold tracking-wide truncate'>{transporter.transporterName || 'Unknown Transporter'}</span>
          ...
          <div className='flex items-center gap-3 text-xs shrink-0'>
            <span className='text-blue-200'>
              {transporter.totalVouchers ?? '-'} voucher{transporter.totalVouchers !== 1 ? 's' : ''}
            </span>
            <span className='text-blue-200'>Qty: <span className='text-white font-semibold'>{(transporter.totalQuantity ?? 0).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span></span>
            <span className='text-white font-bold text-sm'>₹{(transporter.totalAmount ?? 0).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
          </div>
        </div>
      </div>
      ...
      <div className='bg-white'>
        {/* Column header */}
        <div className={cn(`grid ${COLUMN_GRID} text-[11px] font-semibold bg-gray-100 border-b-2 border-gray-300`)}>
          <div className='pl-2 py-1.5 text-left text-gray-600 flex items-center gap-1'>
            Vch No
            <button
              className='text-gray-400 hover:text-gray-600 transition-colors'
              onClick={allExpanded ? collapseAll : expandAll}
              title={allExpanded ? 'Collapse all voucher groups' : 'Expand all voucher groups'}
            >
              <ChevronDownIcon className={`h-3 w-3 transition-transform ${allExpanded ? 'rotate-0' : '-rotate-90'}`} />
            </button>
          </div>
          <div className='border-l border-gray-200 pl-1.5 py-1.5 text-left text-gray-600'>Date</div>
          <div className='border-l border-gray-200 pl-1.5 py-1.5 text-left text-gray-600'>Party</div>
          <div className='border-l border-gray-200 pl-1.5 py-1.5 text-left text-gray-600'>Item</div>
          <div className='border-l border-gray-200 pl-1.5 py-1.5 text-left text-gray-600'>Source</div>
          <div className='border-l border-gray-200 pl-1.5 py-1.5 text-left text-gray-600'>Destination</div>
          <div className='border-l border-gray-200 py-1.5 text-right pr-1 text-gray-600'>Qty</div>
          <div className='border-l border-gray-200 py-1.5 text-right pr-1 text-gray-600'>Amount</div>
          <div className='border-l border-gray-200 py-1.5 text-center text-gray-600'>Status</div>
          <div className='border-l border-gray-200 py-1.5 text-center text-gray-600'></div>
        </div>

        {/* Voucher groups */}
        {voucherGroups.map((group, groupIndex) => {
          const isExpanded = expandedVouchers[group.voucherNo] ?? group.items.length <= 1
          const displayItems = isExpanded ? group.items : group.items.slice(0, 1)
          const hasMoreItems = group.items.length > 1

          return (
            <div key={group.voucherNo} className={cn(
                ...
                return (
                  <div
                    ...
                    className={cn(
                      ...
                      isItemRow ? 'bg-gray-50/40 hover:bg-blue-50/40' : 'hover:bg-gray-50'
                    )}
                  >
                    {/* Vch No */}
                    <div className='pl-2 py-1 text-left font-mono font-semibold text-gray-700 truncate flex items-center gap-1'>
                      {isFirstInGroup && hasMoreItems && (
                        <button
                          ...
                          title={isExpanded ? 'Collapse items' : `Show all ${group.items.length} items`}
                        >
                      ...
                      <span className={cn(
                        isFirstInGroup ? 'text-gray-800 font-bold' : 'text-gray-300',
                        'truncate'
                      )}>
                    ...
                    <div className='border-l border-gray-100 pl-1.5 py-1 text-left text-gray-400 text-[11px] truncate'>
                      {isFirstInGroup && entry.voucherDate ? formatDDMMMYYYY(entry.voucherDate) : ''}
                    </div>

                    {/* Party */}
                    <div className='border-l border-gray-100 pl-1.5 py-1 text-gray-600 truncate text-[11px]' title={isFirstInGroup ? entry.partyName : ''}>
                      {isFirstInGroup ? (
                        <span className='font-medium text-gray-700'>{entry.partyName}</span>
                      ) : ''}
                    </div>

                    {/* Item Name */}
                    <div className='border-l border-gray-100 pl-1.5 py-1 text-gray-700 truncate text-[12px] font-medium' title={entry.itemName}>
                      <span className='inline-flex items-center gap-1'>
                        {isItemRow && <span className='w-1 h-1 rounded-full bg-blue-300 shrink-0' />}
                        {entry.itemName}
                      </span>
                      {entry.unitCode && (
                        <span className='text-[10px] text-gray-400 ml-1 font-normal'>{entry.unitCode}</span>
                      )}
                    </div>
                    ...
                    <div className='border-l border-gray-100 py-1 text-right pr-1.5 text-gray-800 font-mono text-[12px] font-medium tabular-nums'>
                      {(entry.actualQuantity ?? 0).toLocaleString('en-IN', { minimumFractionDigits: entry.noOfDecimalPlaces ?? 2, maximumFractionDigits: entry.noOfDecimalPlaces ?? 2 })}
                    </div>
                    ...
                    <div className='border-l border-gray-100 py-1 text-right pr-1.5 text-gray-800 font-mono text-[12px] font-semibold tabular-nums'>
                      <span className='text-[10px] text-gray-400'>₹</span>{(entry.amount ?? 0).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                    </div>
                    ...
                    <div className='border-l border-gray-100 py-1 flex justify-center'>
                      {isFirstInGroup && entry.paymentStatus ? (
                        <span className={cn(
                          ...
                          entry.paymentStatus === 'paid' ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' :
                          entry.paymentStatus === 'partially_paid' ? 'bg-blue-50 text-blue-700 border border-blue-200' :
                          'bg-amber-50 text-amber-700 border border-amber-200'
                        )}>
                          {entry.paymentStatus === 'paid' ? 'Paid' :
                           entry.paymentStatus === 'partially_paid' ? 'Partial' : 'Unpaid'}
                        </span>
                      ) : (
              ...
              {!isExpanded && hasMoreItems && (
                <div
                  ...
                  onClick={() => toggleVoucher(group.voucherNo)}
                >
                  + {group.items.length - 1} more item{(group.items.length - 1) !== 1 ? 's' : ''} · Click to expand
                </div>
              )}
              ...
              {isExpanded && group.items.length > 1 && (
                <div className={cn(
                  ...
                  <div className='text-right pr-1.5 text-blue-700 font-mono font-semibold tabular-nums'>
                    {group.items.reduce((sum, i) => sum + (i.actualQuantity ?? 0), 0).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                  </div>
                  <div className='text-right pr-1.5 text-blue-700 font-mono font-semibold tabular-nums'>
                    <span className='text-[9px] text-blue-400'>₹</span>
                    {group.items.reduce((sum, i) => sum + (i.amount ?? 0), 0).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                  </div>
                  <div></div>
                  <div></div>
                </div>
        ...
        {(!transporter.entries || transporter.entries.length === 0) && (
          <div className='text-center text-gray-400 py-6 text-xs'>No entries found for this transporter.</div>
        )}
        ...
        {(transporter.entries?.length ?? 0) > entryPageSize && (
          <div className='flex items-center justify-between px-3 py-1.5 bg-gray-50 border-t border-gray-200'>
            <span className='text-[10px] text-gray-500'>
              Showing {(safePage * entryPageSize) + 1}–{Math.min((safePage + 1) * entryPageSize, transporter.entries?.length ?? 0)} of {transporter.entries?.length ?? 0} entries
            </span>
            <div className='flex items-center gap-2'>
              ...
              <div className='flex items-center gap-0.5'>
                <Button variant='ghost' className='h-5 w-5 p-0 text-gray-600 hover:text-gray-900 hover:bg-gray-200'
                  onClick={() => setEntryPage(0)} disabled={safePage === 0}>
                  <DoubleArrowLeftIcon className='h-3 w-3' />
                </Button>
                <Button variant='ghost' className='h-5 w-5 p-0 text-gray-600 hover:text-gray-900 hover:bg-gray-200'
                  onClick={() => setEntryPage((p) => Math.max(0, p - 1))} disabled={safePage === 0}>
                ...
                <Button variant='ghost' className='h-5 w-5 p-0 text-gray-600 hover:text-gray-900 hover:bg-gray-200'
                  onClick={() => setEntryPage((p) => Math.min(totalEntryPages - 1, p + 1))} disabled={safePage >= totalEntryPages - 1}>
                  <ChevronRightIcon className='h-3 w-3' />
                </Button>
                <Button variant='ghost' className='h-5 w-5 p-0 text-gray-600 hover:text-gray-900 hover:bg-gray-200'
                  onClick={() => setEntryPage(totalEntryPages - 1)} disabled={safePage >= totalEntryPages - 1}>
      ...
      <div className='bg-gradient-to-r from-blue-50 to-indigo-50 border-t border-blue-200'>
        <div className='flex items-center justify-between px-3 py-1.5 text-[12px] font-semibold text-blue-800'>
          <div className='flex items-center gap-2'>
            ...
            <span className='tracking-wide'>{transporter.transporterName || 'Unknown'}</span>
          </div>
          <div className='flex items-center gap-4 text-blue-600'>
            <span>Qty: <span className='text-blue-800 font-bold'>{(transporter.totalQuantity ?? 0).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span></span>
            <span className='text-blue-300'>|</span>
            <span>{transporter.totalVouchers ?? 0} voucher{transporter.totalVouchers !== 1 ? 's' : ''}</span>
            <span className='text-blue-300'>|</span>
            <span className='text-blue-800 font-bold'>₹{(transporter.totalAmount ?? 0).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
          </div>
        </div>
      </div>
    </div>
