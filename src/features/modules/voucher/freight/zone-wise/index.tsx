...
export default function FreightZoneWise({
  data,
  chartTitle,
...
const ReportView = ({
  ...
  const grandTotal = useMemo(() => {
    return pageData.reduce(
      (acc, zone) => ({
        ...
        totalInwardQuantity:
          acc.totalInwardQuantity + (zone.totalInwardQuantity ?? 0),
        totalOutwardQuantity:
          acc.totalOutwardQuantity + (zone.totalOutwardQuantity ?? 0),
        totalClosingQuantity:
          acc.totalClosingQuantity + (zone.totalClosingQuantity ?? 0),
  ...
  const grandDp = getQuantityDecimals(
    pageData.flatMap((z) => z.godownDetails ?? [])[0]?.noOfDecimalPlaces,
  )
        ...
        <div className="bg-gray-800 text-white border-t-2 border-gray-900">
          <div className="flex items-center justify-between px-3 py-1.5 text-[12px] font-bold">
            ...
            <span className="text-gray-300">
              In: {(grandTotal.totalInwardQuantity ?? 0).toFixed(grandDp)} |
              Out: {(grandTotal.totalOutwardQuantity ?? 0).toFixed(grandDp)} |
              Cls: {(grandTotal.totalClosingQuantity ?? 0).toFixed(grandDp)}
            </span>
            <span>
...
const ZoneSection = ({ zone }: { zone: ZoneWiseReportItem; index: number }) => {
  ...
  // Quantity decimals for the zone summary row — from the first detail row's
  // stock unit (noOfDecimalPlaces), falling back to 2.
  const zoneDp = getQuantityDecimals(details[0]?.noOfDecimalPlaces)
      ...
      >
        <div className="text-left pl-2 flex items-center gap-2">
          ...
          <span className="text-xs text-gray-600 font-normal ml-auto mr-2">
            {zone.totalEntries ?? '-'} entr
            {zone.totalEntries !== 1 ? 'ies' : 'y'}
          </span>
        </div>
            ...
            >
              <div className="grid grid-cols-[0.7fr_0.7fr_0.8fr_0.8fr_1.1fr_0.5fr_0.65fr_0.55fr_auto] text-[11px] items-center">
                ...
                <div className="text-center text-gray-700 font-medium">
                  {detail.actualQuantity?.toFixed(
                    detail.noOfDecimalPlaces ?? 2,
                  ) ?? '-'}
                </div>
                {/* Amount */}
          ...
          <div className="bg-blue-50 border-t-2 border-blue-300">
            <div className="flex items-center justify-between px-3 py-1 text-[11px] font-semibold text-blue-800">
              ...
              <span className="text-blue-600">
                In: {(zone.totalInwardQuantity ?? 0).toFixed(zoneDp)} | Out:{' '}
                {(zone.totalOutwardQuantity ?? 0).toFixed(zoneDp)} | Cls:{' '}
                {(zone.totalClosingQuantity ?? 0).toFixed(zoneDp)}
              </span>
              <span>
                {zone.totalEntries ?? 0} entr
                {zone.totalEntries !== 1 ? 'ies' : 'y'}
              </span>
