...
import { billingPreferenceSchema } from '../../../data-schema/voucher-schema'
...
export const BillingPreferenceSelector = ({
  form,
  name,
  gapClass,
  label,
  ...
  const frameworks = billingPreferenceSchema.options.map((item: string) => ({
    label: item,
    value: item,
  }))
  ...
  return (
    <div
      className={cn(
        'grid grid-cols-1 items-start space-y-0 gap-x-4 gap-y-2 sm:items-center sm:gap-y-1',
        gapClass,
      )}
    >
        ...
        <SheetContent className="sheet-content-width-same-as-trigger p-0">
          ...
          <Command className="rounded-lg border shadow-md min-w-full">
            ...
            <CommandList className=" max-h-full">
              ...
              <CommandGroup>
                {frameworks.map(
                  (framework: { label: string; value: string }) => (
                    <CommandItem
