...
export default function DeliveryNoteGodownWise({
  zones,
  godowns,
}: DeliveryNoteGodownWiseProps) {
          ...
          <SelectContent>
            {zones.map((zone) => (
              <SelectItem key={zone.id} value={String(zone.id)}>
                ...
                {zone.code && (
                  <span className="text-gray-400 ml-2">({zone.code})</span>
                )}
              </SelectItem>
            ))}
          </SelectContent>
              ...
              <SelectContent>
                {zoneGodowns.map((g) => (
                  <SelectItem key={g.id} value={String(g.id)}>
                    <span>{g.name}</span>
                    {g.code && (
                      <span className="text-gray-400 ml-2">({g.code})</span>
                    )}
                  </SelectItem>
                ))}
              </SelectContent>
      ...
      {!selectedZoneId ? (
        <div className="flex flex-col items-center justify-center py-16 text-gray-400">
          ...
          <p className="text-base font-medium">
            Select a zone to view delivery note details
          </p>
          <p className="text-sm mt-1">
            Choose a zone from the dropdown above to see godown-wise delivery
            notes.
          </p>
        </div>
      ) : isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-16 text-gray-400">
          ...
          <p className="text-sm mt-1">
            No delivery note data available for{' '}
...
const ReportView = ({
  ...
  data: Array<GodownWiseReportItem>
  zoneName?: string
}) => {
  ...
  const grandTotal = useMemo(() => {
    return data.reduce(
      (acc, gd) => ({
        ...
        totalInwardQuantity:
          acc.totalInwardQuantity + (gd.totalInwardQuantity ?? 0),
        totalOutwardQuantity:
          acc.totalOutwardQuantity + (gd.totalOutwardQuantity ?? 0),
        totalClosingQuantity:
          acc.totalClosingQuantity + (gd.totalClosingQuantity ?? 0),
  ...
  // Quantity decimals for aggregate rows — derived from the first detail row's
  // stock unit (noOfDecimalPlaces), falling back to 2.
  const grandDp = getQuantityDecimals(
    data.flatMap((g) => g.voucherDetails ?? [])[0]?.noOfDecimalPlaces,
  )
        ...
        <div className="bg-gray-800 text-white border-t-2 border-gray-900">
          <div className="flex items-center justify-between px-3 py-1.5 text-[12px] font-bold">
            <span className="tracking-wide">
              GRAND TOTAL — {zoneName ?? ''}
            </span>
            <span className="text-gray-300">
              In: {formatQty(grandTotal.totalInwardQuantity, grandDp)} | Out:{' '}
              {formatQty(grandTotal.totalOutwardQuantity, grandDp)} | Cls:{' '}
              {formatQty(grandTotal.totalClosingQuantity, grandDp)}
            </span>
            <span>
              {grandTotal.totalEntries ?? 0} entr
              {grandTotal.totalEntries !== 1 ? 'ies' : 'y'}
            </span>
...
const GodownSection = ({ godown }: { godown: GodownWiseReportItem }) => {
  ...
  // Quantity decimals for the godown summary row — from the first detail row's
  // stock unit (noOfDecimalPlaces), falling back to 2.
  const godownDp = getQuantityDecimals(details[0]?.noOfDecimalPlaces)
  const totalVoucherPages = Math.max(
    1,
    Math.ceil(details.length / voucherPageSize),
  )
  const safePage = Math.min(voucherPage, totalVoucherPages - 1)
  const pageDetails = details.slice(
    safePage * voucherPageSize,
    (safePage + 1) * voucherPageSize,
  )
      ...
      <div className="grid grid-cols-[1.5fr_1.5fr] text-center font-semibold bg-gray-300 shadow-md">
        <div className="text-left pl-2 flex items-center gap-2">
          ...
          <span className="text-xs text-gray-600 font-normal ml-auto mr-2">
            {godown.totalEntries ?? '-'} entr
            {godown.totalEntries !== 1 ? 'ies' : 'y'}
          </span>
        </div>
          ...
          {details.length > 0 && (
            <div className="flex items-center gap-1">
              ...
              <Select
                ...
                onValueChange={(value) =>
                  handleVoucherPageSizeChange(Number(value))
                }
              >
          ...
          {totalVoucherPages > 1 && (
            <div className="flex items-center gap-0.5">
              ...
              <Button
                ...
                className="h-5 w-5 p-0 text-gray-600 hover:text-gray-900"
                onClick={() =>
                  setVoucherPage((p) => Math.min(totalVoucherPages - 1, p + 1))
                }
          ...
          {pageDetails.map((detail, detailIndex) => (
            ...
            >
              <div className="grid grid-cols-[0.7fr_0.7fr_0.8fr_0.8fr_1.1fr_0.5fr_0.65fr_0.55fr_auto] text-[11px] items-center">
                ...
                <div
                  className="pl-1 text-gray-600 truncate"
                ...
                <div
                  ...
                  title={`${detail.itemName ?? ''} / ${detail.godownName ?? ''}`}
                >
                ...
                <div
                  className="pl-1 text-[10px] text-muted-foreground truncate"
                  title={buildDispatchLabel(detail)}
                >
                  {buildDispatchLabel(detail)}
                </div>
                <div className="text-center text-gray-700 font-medium">
                  {formatQty(detail.actualQuantity, detail.noOfDecimalPlaces)}
                </div>
          ...
          <div className="bg-blue-50 border-t-2 border-blue-300">
            <div className="flex items-center justify-between px-3 py-1 text-[11px] font-semibold text-blue-800">
              ...
              <span className="text-blue-600">
                In: {formatQty(godown.totalInwardQuantity, godownDp)} | Out:{' '}
                {formatQty(godown.totalOutwardQuantity, godownDp)} | Cls:{' '}
                {formatQty(godown.totalClosingQuantity, godownDp)}
              </span>
              <span>
                {godown.totalEntries ?? 0} entr
                {godown.totalEntries !== 1 ? 'ies' : 'y'}
              </span>
