...
import {
  storeUniqueQuantityCodeService,
  updateUniqueQuantityCodeService,
} from '../data/api'
...
export function UniqueQuantityCodeActionDialog({
  currentRow,
  open,
  onOpenChange,
}: Props) {
  ...
  const mutateUniqueQuantityCode = useMutation({
    mutationFn: async (data: UniqueQuantityCodeForm) => {
      ...
      if (isEdit && currentRow) {
        return await updateUniqueQuantityCodeService({
          ...data,
          id: currentRow.id,
        })
      } else if (!isEdit) {
    ...
    >
      <DialogContent className="sm:max-w-lg">
        <DialogHeader className="text-left border-b-2 pb-2">
          <DialogTitle>
            {isEdit
              ? 'Edit Unique Quantity Code'
              : 'Add New Unique Quantity Code'}
          </DialogTitle>
          <DialogDescription>
            {isEdit
              ? 'Update the Unique Quantity Code here. '
              : 'Create new Unique Quantity Code here. '}
