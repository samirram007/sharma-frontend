...
export default function StepStockQuantities({ stockItems, onNext, onBack }: StepStockQuantitiesProps) {
  ...
  const updateQuantity = (itemId: number, godownId: number, quantity: number) => {
    setEntries((prev) =>
      prev.map((item) =>
        item.itemId === itemId
          ? {
          ...
          {stockItems.length === 0 ? (
            ...
            <div className='max-h-[500px] overflow-y-auto space-y-2'>
              {entries.map((entry) => {
                ...
                const isExpanded = expandedItems.has(entry.itemId) || totalQty > 0
                      ...
                      <div className='flex items-center gap-3'>
                        {totalQty > 0 && (
                          <span className='text-sm font-mono text-muted-foreground'>
                            Total: {totalQty.toFixed(entry.noOfDecimalPlaces)} {entry.unitCode}
                          </span>
                        )}
                        <svg
                          className={cn(
