        ...
        <CardHeader>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <CardDescription>
                {result.voucherNo && `Voucher: ${result.voucherNo}`}
              </CardDescription>
            </div>
          </div>
        </CardHeader>
        ...
        <CardContent className="space-y-4">
          {!hasAnything && (
            <div className="py-8 text-center text-muted-foreground">
              No ledger balances or stock quantities entered. Go back and add
              some data.
            </div>
          )}
          ...
          {activeStockItems.length > 0 && (
            <div>
              <h4 className="mb-2 flex items-center gap-2 text-sm font-semibold">
                ...
                Stock Quantities ({activeStockItems.length} items,{' '}
                {totalStockQty.toFixed(2)} total qty)
              </h4>
              <div className="max-h-[200px] overflow-y-auto rounded-md border">
                <table className="w-full text-sm">
                  <thead className="sticky top-0 bg-muted/80 backdrop-blur-sm">
                  ...
                  <tbody>
                    {activeStockItems.map((entry) => {
                      const activeGodowns = entry.godowns.filter(
                        (g) => g.quantity > 0,
                      )
                      return activeGodowns.map((godown, gi) => (
                        <tr
                          key={`${entry.itemId}-${godown.godownId}`}
                          className="border-b last:border-0"
                        >
                          {gi === 0 && (
                            <td
