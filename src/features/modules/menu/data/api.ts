...
export async function patchMenuService(
  id: number,
  payload: Partial<{
    menu_name?: string
    is_visible?: boolean
...
export async function reorderMenuService(
...
export async function batchUpdateMenuService(
  ids: number[],
...
export async function searchMenuService(search: string, perPage = 20) {
  return await getData(
    `/menus/search?search=${encodeURIComponent(search)}&per_page=${perPage}`,
  )
}
